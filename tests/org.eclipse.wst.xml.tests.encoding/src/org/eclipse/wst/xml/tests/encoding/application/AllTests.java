...
public class AllTests extends TestCase {
	private final static String INTERACTIVE = "-interactive"; //$NON-NLS-1$
	private final static String VARY_ENCODING = "-varyEncoding"; //$NON-NLS-1$
	private final static String PERFORMANCE = "-performance"; //$NON-NLS-1$
	private final static String MEMORY = "-memory"; //$NON-NLS-1$
	...
	public Object runMain(Object args) {
		...
		try {
			...
			if (!interactive) {
				System.out.println();
				System.out.println("Tests running ... output will be saved to file"); //$NON-NLS-1$
			...
			if (interactive) {
				if (!Platform.inDevelopmentMode()) {
					pause("press any key to continue"); //$NON-NLS-1$
				}
			}
			else {
				...
				System.out.println("Full output in \n     " + outFile.getAbsolutePath()); //$NON-NLS-1$
			}
		}
		catch (Exception e) {
			result = e;
	...
	protected void printVMInfo() {
		String vminfo = System.getProperty("java.vm.info"); //$NON-NLS-1$
		String vmname = System.getProperty("java.vm.name"); //$NON-NLS-1$
		System.out.println();
		System.out.println("Sed tests running in " + vmname); //$NON-NLS-1$
		System.out.println("   " + vminfo); //$NON-NLS-1$
		System.out.println();
	}

	public static Test suite() {
		TestSuite testSuite = new TestSuite();
		testSuite.addTest(new EncodingTestSuite());
		return testSuite;
	}
	...
	protected void printHeader(TestResult result) {
		if (result.wasSuccessful()) {
			...
			System.out.println(" (" + result.runCount() + " tests)"); //$NON-NLS-1$ //$NON-NLS-2$

		}
		else {
			System.out.println();
			System.out.println("FAILURES!!!"); //$NON-NLS-1$
			System.out.println("Tests run: " + result.runCount() + ",  Failures: " + result.failureCount() + ",  Errors: " + result.errorCount()); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		}
	}
}