...
/**
 * This job holds a queue of updates from the editor (DirtyRegions) to process.
 * When a new request comes in, the current run is canceled, the new request is
 * added to the queue, then the job is re-scheduled.
 * 
 * @author pavery
 */
public class DirtyRegionProcessor extends Job implements IReconciler {
     ...
     * strategies/steps. Currently used when widget is disposed (user is trying
     * to close the editor), and on uninstall.
     * 
