 ...
 *
 * <p>Usage: fn:translate($arg as xs:string?,
 *                     $mapString as xs:string,
 *                     $transString as xs:string) as xs:string</p>
 *
 * <p>This class returns the value of $arg modified so that every character in the value of $arg
 * that occurs at some position N in the value of $mapString has been replaced by the
 * character that occurs at position N in the value of $transString.</p>
 *
 * <p>If the value of $arg is the empty sequence, the zero-length string is returned.</p>
 *
 * <p>Every character in the value of $arg that does not appear in the value of $mapString is
 * unchanged.</p>
 *
 * <p>Every character in the value of $arg that appears at some position M in the value of 
 * $mapString, where the value of $transString is less than M characters in length, is
 * omitted from the returned value. If $mapString is the zero-length string $arg is returned.</p>
 *
 * <p>If a character occurs more than once in $mapString, then the first occurrence determines
 * the replacement character. If $transString is longer than $mapString, the excess characters
 * are ignored.</p>
 */
