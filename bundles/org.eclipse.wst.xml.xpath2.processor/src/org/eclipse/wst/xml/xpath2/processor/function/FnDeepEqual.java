 ...
 * and for two items to be deep-equal, they must either by atomic values that
 * compare equal, or nodes of the same kind, with the same name, whose children
 * are deep-equal. This is defined in more detail below. The $collation argument
...
public class FnDeepEqual extends Function {
	...
	/**
         ...
         * @param one input1 xpath expression/variable.
         * @param two input2 xpath expression/variable.
         * @return Result of fn:deep-equal operation.
         */
	...
	public static boolean deep_equal(AnyType one, AnyType two) {
		if( (one instanceof AnyAtomicType) && 
		    (two instanceof AnyAtomicType))
		    return deep_equal( (AnyAtomicType) one, (AnyAtomicType) two);
		    
		else if( ( (one instanceof AnyAtomicType) && 
		           (two instanceof NodeType) ) ||
			 ( (one instanceof NodeType) &&
			   (two instanceof AnyAtomicType) ) )
		     return false;
