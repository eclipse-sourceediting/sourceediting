...
public class DefaultEvaluator implements XPathVisitor, Evaluator {
	...
	private ResultSequence do_for_quantified_expr(ListIterator iter, 
		...
		if(iter.hasNext()) {
			...
			for(Iterator i = rs.iterator(); i.hasNext();) {
				...
				_dc.new_scope();
				ResultSequence res = do_for_quantified_expr(iter, finalexpr, type);
				_dc.destroy_scope();
				assert res != null;
	...
	public Object visit(QuantifiedExpr qex) {
		...
		ResultSequence rs = do_for_quantified_expr(pairs.listIterator(), qex.expr(), hack);
...
//		ResultSequence rs = (ResultSequence) cexp.left().accept(this);
		...
		try {
			CastExpr ce = new CastExpr((Expr) cexp.left(), 
						   (SingleType) cexp.right());
...
*/				
	...
	public Object visit(ElementTest e) {
		...
		if(name != null && !e.wild()) {
			for(Iterator i = rs.iterator(); i.hasNext();) {
				if(!name_test((ElementType)i.next(),
					      name, "element"))
				...
				if(xpe.next() == null && xpe.slashes() == 0 &&
				   xpe.expr() instanceof FilterExpr) {
					...
					if(fex.primary() instanceof IntegerLiteral) {
						int pos = ((IntegerLiteral)fex.primary()).value().int_value();
