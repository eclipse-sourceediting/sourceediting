...
public class Normalizer implements XPathVisitor {
	...
	private void doForExpr(Iterator iter, Expr expr) {
		...
		while(iter.hasNext()) {
			...
			// [shadow outer vars]
/*			
			if(_sc.variable_exists(var))
				report_error(new StaticNameError("Variable " +
						  var.string() + 
						  " already defined"));
*/						  
	...
	private BinExpr make_logic_expr(BinExpr e) {
		...
		e.set_left(make_function(new QName("fn","boolean",
					 FnFunctionLibrary.XPATH_FUNCTIONS_NS),
					 args));
	...
	private XPathExpr make_convert_binop(BinExpr e, XPathExpr convarg, QName name) {
		...
		for(j = 0; j < 2; j++) {
			XPathExpr arg = make_convert_operand(args_arr[j],
			                                     convarg);
			argsfname.add(arg);
		}
	...
	/**
 	 * @param mulex is the integer division expression that always returns an integer.
	 * @return a new function.
 	 */
	// XXX: integer cast!
