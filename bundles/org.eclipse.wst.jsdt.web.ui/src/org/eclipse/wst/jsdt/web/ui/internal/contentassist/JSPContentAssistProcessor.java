...
//		fNameToProcessorMap.put(JSP11Namespace.ElementName.SETPROPERTY,
//				jspPropertyCAP);
//		fNameToProcessorMap.put(JSP11Namespace.ElementName.GETPROPERTY,
//				jspPropertyCAP);
//		fNameToProcessorMap.put(JSP11Namespace.ElementName.USEBEAN,
//				new JSPUseBeanContentAssistProcessor());
//		fNameToProcessorMap.put(JSP11Namespace.ElementName.DIRECTIVE_TAGLIB,
//				new JSPTaglibDirectiveContentAssistProcessor());
	}
	...
	protected void initPartitionToProcessorMap() {
		...
		fPartitionToProcessorMap.put(IStructuredPartitions.DEFAULT_PARTITION,
				htmlProcessor);
		fPartitionToProcessorMap.put(IJSPPartitions.JSP_DEFAULT,
				jspJavaProcessor);
...
//				javascriptProcessor); // default
	...
	protected void addAttributeValueProposals(
		...
		addTemplates(contentAssistRequest,
				TemplateContextTypeIdsJSP.ATTRIBUTE_VALUE);
		...
		if (mq != null) {
			...
			if (doc instanceof JSPCMDocument || doc instanceof CMNodeWrapper
					|| node.getNodeName().startsWith("jsp:")) {
				return;
			}
		}
		...
		if (attributeName != null) {
			currentValue = node.getAttributes().getNamedItem(attributeName)
					.getNodeValue();
		}
		...
		if ((contentAssistRequest.getRegion().getType() == DOMRegionContext.XML_TAG_ATTRIBUTE_EQUALS && contentAssistRequest
			...
			List additionalElements = getAdditionalChildren(new ArrayList(),
					node, -1);
			for (i = 0; i < additionalElements.size(); i++) {
				CMElementDeclaration ed = (CMElementDeclaration) additionalElements
						.get(i);
				if (rejectElements.contains(ed.getNodeName())) {
					continue;
				}
				String tagname = getContentGenerator()
						.getRequiredName(node, ed);
				StringBuffer contents = new StringBuffer("\""); //$NON-NLS-1$
				getContentGenerator().generateTag(node, ed, contents);
				contents.append('"');
				CustomCompletionProposal proposal = new CustomCompletionProposal(
						contents.toString(), contentAssistRequest
								.getReplacementBeginPosition(),
						contentAssistRequest.getReplacementLength(), contents
								.length(),
						JSPEditorPluginImageHelper.getInstance().getImage(
								JSPEditorPluginImages.IMG_OBJ_TAG_GENERIC),
						tagname, null, null,
			...
			try {
				...
				IModelManager mmanager = StructuredModelManager
						.getModelManager();
				...
				if (contentAssistRequest.getNode() instanceof IDOMNode) {
					...
					internalModel.setBaseLocation(xmlOuterModel
							.getBaseLocation());
				}
				String contents = StringUtils.strip(contentAssistRequest
						.getText());
...
//					TLDCMDocumentManager mgr = TaglibController
//							.getTLDCMDocumentManager(xmlOuterModel
//									.getStructuredDocument());
//					if (mgr != null) {
//						List trackers = mgr
//								.getCMDocumentTrackers(contentAssistRequest
//										.getReplacementBeginPosition());
//						if (trackers != null) {
//							for (i = 0; i < trackers.size(); i++) {
//								CMDocumentTracker tracker = (CMDocumentTracker) trackers
//										.get(i);
...
//					}
					...
					NodeList useBeans = doc
							.getElementsByTagName(JSP12Namespace.ElementName.USEBEAN);
					for (int k = 0; k < useBeans.getLength(); k++) {
						IDOMNode useBean = (IDOMNode) useBeans.item(k);
						if (useBean.getStartOffset() < contentAssistRequest
								.getReplacementBeginPosition()) {
							StringBuffer useBeanText = new StringBuffer(
									"<jsp:useBean"); //$NON-NLS-1$
							for (int j = 0; j < useBean.getAttributes()
									.getLength(); j++) {
								Attr attr = (Attr) useBean.getAttributes()
										.item(j);
								useBeanText.append(' ');
					...
					if (contentAssistRequest.getMatchString().length() > 0
						...
						internalOffset = contentAssistRequest.getMatchString()
								.length()
								- 1 + additionalShifts;
						quoted = true;
					}
					// if unquoted, use position inside
						...
						internalOffset = contentAssistRequest
								.getReplacementBeginPosition()
					...
					while (internalNode == null && tmpOffset >= 0) {
						internalNode = internalModel
								.getIndexedRegion(tmpOffset--);
					}

					if (internalModel.getFactoryRegistry() != null) {
						// set up the internal model
						if (internalModel.getFactoryRegistry().getFactoryFor(
								PageDirectiveAdapter.class) == null) {
							internalModel.getFactoryRegistry().addFactory(
									new PageDirectiveAdapterFactory());
						}
						PageDirectiveAdapter outerEmbeddedTypeAdapter = (PageDirectiveAdapter) xmlOuterModel
								.getDocument().getAdapterFor(
										PageDirectiveAdapter.class);
						...
						internalEmbeddedTypeAdapter
								.setEmbeddedType(outerEmbeddedTypeAdapter
										.getEmbeddedType());
					}

					AdapterFactoryRegistry adapterRegistry = JSPUIPlugin
							.getDefault().getAdapterFactoryRegistry();
					Iterator adapterList = adapterRegistry
							.getAdapterFactories();
					// And all those appropriate for this particular type
					// of content
					while (adapterList.hasNext()) {
						try {
							AdapterFactoryProvider provider = (AdapterFactoryProvider) adapterList
									.next();
	...
	private List getAdditionalChildren(List elementDecls, Node node,
			int childIndex) {
		if (node instanceof IDOMNode) {
			...
			if (children.getLength() >= childIndex && childIndex >= 0) {
				...
				if (nodeAlreadyAtIndex instanceof IDOMNode) {
					textInsertionOffset = ((IDOMNode) nodeAlreadyAtIndex)
							.getEndOffset();
				}
			} else {
...
//			TLDCMDocumentManager mgr = TaglibController
//					.getTLDCMDocumentManager(((IDOMNode) node)
//							.getStructuredDocument());
//			if (mgr != null) {
//				List moreCMDocuments = mgr
//						.getCMDocumentTrackers(textInsertionOffset);
...
//								CMElementDeclaration ed = (CMElementDeclaration) elements
//										.item(j);
//								elementDecls.add(ed);
//							}
//						}
...
//			}
			...
			if (node.getNodeType() == Node.DOCUMENT_NODE) {
				mqAdapter = (ModelQueryAdapter) ((IDOMNode) node)
			...
			if (mqAdapter != null) {
				CMDocument doc = mqAdapter.getModelQuery()
						.getCorrespondingCMDocument(node);
				if (doc != null) {

					CMDocument JCMDoc = HTMLCMDocumentFactory
					...
					if (jspelements != null) {
						...
						rejectElements
								.add(JSP12Namespace.ElementName.SCRIPTLET);
						rejectElements
								.add(JSP12Namespace.ElementName.EXPRESSION);
						rejectElements
								.add(JSP12Namespace.ElementName.DECLARATION);
						rejectElements
								.add(JSP12Namespace.ElementName.DIRECTIVE_INCLUDE);
						rejectElements
								.add(JSP12Namespace.ElementName.DIRECTIVE_PAGE);
						rejectElements.add(JSP12Namespace.ElementName.TEXT);
						rejectElements
								.add(JSP12Namespace.ElementName.DIRECTIVE_TAGLIB);

						if (isXMLFormat(domDoc)) {

							// jsp actions
							rejectElements
									.add(JSP12Namespace.ElementName.FALLBACK);
							rejectElements
									.add(JSP12Namespace.ElementName.USEBEAN);
							rejectElements
									.add(JSP12Namespace.ElementName.GETPROPERTY);
							rejectElements
									.add(JSP12Namespace.ElementName.SETPROPERTY);
							rejectElements
									.add(JSP12Namespace.ElementName.INCLUDE);
							rejectElements
									.add(JSP12Namespace.ElementName.FORWARD);
							rejectElements
									.add(JSP12Namespace.ElementName.PLUGIN);
							rejectElements
									.add(JSP12Namespace.ElementName.FALLBACK);
							rejectElements
									.add(JSP12Namespace.ElementName.PARAM);
							rejectElements
									.add(JSP12Namespace.ElementName.PARAMS);
						}
						...
						if (docElement != null
								&& ((docElement.getNodeName()
										.equals("jsp:root")) || ((((IDOMNode) docElement)
										.getStartStructuredDocumentRegion() != null || ((IDOMNode) docElement)
										.getEndStructuredDocumentRegion() != null)))) {
							rejectElements.add(JSP12Namespace.ElementName.ROOT);
						}

						for (int j = 0; j < jspelements.getLength(); j++) {
							CMElementDeclaration ed = (CMElementDeclaration) jspelements
									.item(j);
							if (rejectElements.contains(ed.getNodeName())) {
								continue;
							}
	...
	protected List getAvailableChildrenAtIndex(Element parent, int index,
		...
		for (int i = 0; i < additionalElements.size(); i++) {
			ModelQueryAction insertAction = new NonValidatingModelQueryAction(
					(CMElementDeclaration) additionalElements.get(i),
					ModelQueryAction.INSERT, 0, parent.getChildNodes()
							.getLength(), null);
		...
		while (iterator.hasNext()) {
			...
			if (action.getCMNode() instanceof HTMLElementDeclaration) {
				HTMLElementDeclaration ed = (HTMLElementDeclaration) action
						.getCMNode();
				String ommission = (String) ed
						.getProperty(HTMLCMProperties.OMIT_TYPE);
				if (!childNames.contains(ed.getNodeName().toLowerCase())
						&& ((ommission != null) && (ommission
								.equals(HTMLCMProperties.Values.OMIT_BOTH)))) {
					List implicitValidActions = new ArrayList();
					modelQuery.getInsertActions(parent, ed, 0,
							ModelQuery.INCLUDE_CHILD_NODES,
							ModelQuery.VALIDITY_NONE, implicitValidActions);
					if (implicitValidActions != null) {
						Iterator implicitValidActionsIterator = implicitValidActions
								.iterator();
						while (implicitValidActionsIterator.hasNext()) {
							ModelQueryAction insertAction = new NonValidatingModelQueryAction(
									((ModelQueryAction) implicitValidActionsIterator
											.next()).getCMNode(),
									ModelQueryAction.INSERT, 0, parent
	...
	public ICompletionProposal[] computeCompletionProposals(ITextViewer viewer,
			...
			if (regions.size() > 1) {
				...
				if (xmlOpenOrClose.getType() == DOMRegionContext.XML_TAG_OPEN
					...
					if (name.equals("jsp:scriptlet") || name.equals("jsp:expression") || name.equals("jsp:declaration")) { //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
						return getJSPJavaCompletionProposals(viewer,
								documentPosition);
					}
				}
			}
		...
		if (partitionType == IJSPPartitions.JSP_CONTENT_JAVASCRIPT
			...
			IStructuredDocumentRegion decodedSDRegion = decodeScriptBlock(fn
					.getFullText());
			...
			if (decodedSDRegion != null) {
				...
				while (sdr != null) {
					...
					if (sdr.getType() == DOMJSPRegionContexts.JSP_CONTENT) {
						if (documentPosition >= fn.getStartOffset()
								+ sdr.getStartOffset()
						...
						} else if (documentPosition == fn.getStartOffset()
								+ sdr.getEndOffset()
								&& sdr.getNext() != null
		...
		if (fn != null && partitionType == IJSPPartitions.JSP_CONTENT_DELIMITER) {
			...
			while (blockRegions.hasNext()) {
				...
				if (temp instanceof ITextRegionContainer) {
					...
					// it's in this region
					if (documentPosition >= trc.getStartOffset()
							&& documentPosition < trc.getEndOffset()) {
						Iterator nestedJSPRegions = trc.getRegions().iterator();
						while (nestedJSPRegions.hasNext()) {
							temp = (ITextRegion) nestedJSPRegions.next();
							if (XMLContentAssistUtilities
									.isJSPOpenDelimiter(temp.getType())
									&& documentPosition == trc
											.getStartOffset(temp)) {
								// HTML content assist
								...
								if (documentPosition > 0) {
									...
									.isJSPCloseDelimiter(temp.getType())
									&& documentPosition == trc
											.getStartOffset(temp)) {
								// JSP content assist
								return getJSPJavaCompletionProposals(viewer,
			...
			if (XMLContentAssistUtilities.isXMLJSPDelimiter(fnDelim)) {
				...
				if (fnDelim.getStartOffset() == documentPosition
					...
					if (documentPosition > 0) {
						String checkType = getPartitionType(
								(StructuredTextViewer) viewer,
								documentPosition - 1);
				...
				if (XMLContentAssistUtilities.isJSPCloseDelimiter(fnDelim)) {
					if (documentPosition == fnDelim.getStartOffset()) {
						...
						.isJSPOpenDelimiter(fnDelim)) {
					// if it's the first position of open delimiter
					// use embedded HTML results
					if (documentPosition == fnDelim.getStartOffset()) {
						embeddedResults = getHTMLCompletionProposals(viewer,
								documentPosition);
		...
		// <!-- <% |%> -->
		if (fn != null
				&& (fn.getType() == DOMRegionContext.XML_CDATA_TEXT || fn
						.getType() == DOMRegionContext.XML_COMMENT_TEXT)) {
			if (fn instanceof ITextRegionContainer) {
				...
				for (int i = 0; i < cdataRegions.length; i++) {
					...
					if (r instanceof ITextRegionContainer) {
						...
						Object[] jspRegions = ((ITextRegionContainer) r)
								.getRegions().toArray();
						for (int j = 0; j < jspRegions.length; j++) {
							...
							if (jspRegion.getType() == DOMJSPRegionContexts.JSP_CLOSE) {
								if (sdRegion.getStartOffset(jspRegion) == documentPosition) {
									return getJSPJavaCompletionProposals(
		...
		// proposal
		ITextRegion attrContainer = (fn != null) ? fn
				.getRegionAtCharacterOffset(documentPosition) : null;
		if (attrContainer != null
				&& attrContainer instanceof ITextRegionContainer) {
			if (attrContainer.getType() == DOMRegionContext.XML_TAG_ATTRIBUTE_VALUE) {
				...
				// delimeter
				Iterator attrRegions = ((ITextRegionContainer) attrContainer)
						.getRegions().iterator();
				ITextRegion testRegion = null;
				while (attrRegions.hasNext()) {
					testRegion = (ITextRegion) attrRegions.next();
					// need to check for other valid attribute regions
					if (XMLContentAssistUtilities.isJSPOpenDelimiter(testRegion
							.getType())) {
						if (!(((ITextRegionContainer) attrContainer)
								.getEndOffset(testRegion) <= documentPosition)) {
							return EMPTY_PROPOSAL_SET;
						}
					} else if (XMLContentAssistUtilities
							.isJSPCloseDelimiter(testRegion.getType())) {
						if (!(((ITextRegionContainer) attrContainer)
								.getStartOffset(testRegion) >= documentPosition)) {
							return EMPTY_PROPOSAL_SET;
						}
					}
				}
				// TODO: handle non-Java code such as nested tags
				if (testRegion.getType().equals(
		...
		if (p != null) {
			...
			if (partitionType == IJSPPartitions.JSP_CONTENT_JAVASCRIPT
				...
				ICompletionProposal[] beanResults = getJSPJavaBeanProposals(
						viewer, documentPosition);
				if (beanResults != null && beanResults.length > 0) {
					...
					System.arraycopy(beanResults, 0, added, 0,
							beanResults.length);
					System.arraycopy(embeddedResults, 0, added,
							beanResults.length, embeddedResults.length);
		...
		if (!( p instanceof CSSContentAssistProcessor)) {
			fTemplateContexts.clear();
			jspResults = super.computeCompletionProposals(viewer,
					documentPosition);
		}
		...
		if (partitionType == IJSPPartitions.JSP_COMMENT) {
			if (sdRegion.getStartOffset() == documentPosition) {
				ICompletionProposal[] htmlResults = getHTMLCompletionProposals(
						viewer, documentPosition);
		...
		if (partitionType == IJSPPartitions.JSP_DIRECTIVE) {
			ICompletionProposal[] importProposals = getImportProposals(viewer,
					documentPosition);
	...
	private ICompletionProposal[] getImportProposals(ITextViewer viewer,
			int documentPosition) {
		List importProposals = new ArrayList();
		ICompletionProposal[] proposals = getJSPJavaCompletionProposals(viewer,
				documentPosition);
	...
	private ICompletionProposal adjustImportProposal(
			JSPCompletionProposal importProposal) {
	 ...
	 * @return
	 */
	private ICompletionProposal[] merge(ICompletionProposal[] jspResults,
			ICompletionProposal[] htmlResults) {
		List results = new ArrayList();
	...
	private ICompletionProposal[] getJSPJavaBeanProposals(ITextViewer viewer,
			int documentPosition) {
		ICompletionProposal[] regularJSPResults = getJSPJavaCompletionProposals(
				viewer, documentPosition);
		...
		for (int i = 0; i < regularJSPResults.length; i++) {
			...
			System.out
					.println("relevance > " + ((CustomCompletionProposal) test).getRelevance()); //$NON-NLS-1$

			if (isRelevanceAllowed(((CustomCompletionProposal) test)
					.getRelevance())) {
	 ...
	 * @return ICompletionProposal[]
	 */
	protected ICompletionProposal[] getJSPJavaCompletionProposals(
			ITextViewer viewer, int documentPosition) {
	...
	public IContextInformation[] computeContextInformation(ITextViewer viewer,
			int documentOffset, IndexedRegion indexedNode) {
	...
	protected ContentAssistRequest computeCompletionProposals(
		...
		ContentAssistRequest request = super.computeCompletionProposals(
				...
				xmlnode);
		IStructuredDocumentRegion sdRegion = ContentAssistUtils
				.getStructuredDocumentRegion(fTextViewer, documentPosition);
		...
		if (completionRegion.getType() == DOMRegionContext.XML_COMMENT_TEXT
				&& !isXMLFormat(doc)) {
			if (request == null) {
				request = newContentAssistRequest(treeNode, xmlnode, sdRegion,
						completionRegion, documentPosition, 0, ""); //$NON-NLS-1$
			}
			request
					.addProposal(new CustomCompletionProposal(
							"<%=  %>", documentPosition, 0, 4, JSPEditorPluginImageHelper.getInstance().getImage(JSPEditorPluginImages.IMG_OBJ_TAG_GENERIC), "jsp:expression", null, "&lt;%= %&gt;", XMLRelevanceConstants.R_JSP)); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ 
		}
		...
		// JSP_DIRECTIVE_NAME
			...
			if (completionRegion.getType() == DOMJSPRegionContexts.JSP_DIRECTIVE_OPEN) {
				if (request == null) {
					request = newContentAssistRequest(xmlnode, xmlnode,
				...
				for (int i = 0; i < directiveNames.length; i++) {
					if (directiveNames[i].startsWith(nameString)
						...
						request
								.addProposal(new CustomCompletionProposal(
										...
										begin,
										length,
										directiveNames[i].length(),
										JSPEditorPluginImageHelper
												.getInstance()
												.getImage(
														JSPEditorPluginImages.IMG_OBJ_TAG_GENERIC),
										directiveNames[i], null, null,
				...
				if (request == null) {
					request = newContentAssistRequest(xmlnode, xmlnode,
							sdRegion, completionRegion, sdRegion
				...
				|| (completionRegion.getType() == DOMJSPRegionContexts.JSP_DIRECTIVE_CLOSE && documentPosition <= sdRegion
						.getStartOffset(completionRegion))) {
			if (request == null) {
				request = computeAttributeProposals(documentPosition,
			...
			if (nameString == null) {
				for (int i = 0; i < directiveNames.length; i++) {
					request.addProposal(new CustomCompletionProposal(
							directiveNames[i], request
									.getReplacementBeginPosition(), request
									.getReplacementLength(), directiveNames[i]
									.length(),
							JSPEditorPluginImageHelper.getInstance().getImage(
									JSPEditorPluginImages.IMG_OBJ_TAG_GENERIC),
							directiveNames[i], null, null,
