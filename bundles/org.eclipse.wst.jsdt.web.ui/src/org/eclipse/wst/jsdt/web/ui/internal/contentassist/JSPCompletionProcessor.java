	...
	public ICompletionProposal[] computeCompletionProposals(ITextViewer viewer,
		...
		try {
			...
			xmlModel = (IDOMModel) StructuredModelManager.getModelManager()
					.getExistingModelForRead(fViewer.getDocument());
			...
			if (fTranslationAdapter != null) {
				...
				JSPTranslation translation = fTranslationAdapter
						.getJSPTranslation();
				fJavaPosition = translation
						.getJavaOffset(getDocumentPosition());

				if (DEBUG) {
					System.out.println("Cursor Location in Java Offset:"
							+ fJavaPosition);
				}
	...
	private String debug(JSPTranslation translation) {
		...
		while (it.hasNext()) {
			try {
				...
				debugString.append("char at Java pos (-1)>"
						+ javaText.substring(javaPos.offset,
								javaPos.offset + javaPos.length).charAt(
								fJavaPosition - 1) + ":\n");
				debugString.append("char at JSP pos (-1)>"
						+ jspText.charAt(jspCursPos - 1) + ":\n");
