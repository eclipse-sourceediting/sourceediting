 ...
 * @plannedfor 1.0
 */
public class JSPUseBeanContentAssistProcessor extends
	...
	protected void addAttributeValueProposals(
		...
		if (existingComplicatedValue) {
			...
			if (attributeName.equals(JSP11Namespace.ATTR_NAME_CLASS)) {
				...
				ICompletionProposal[] classProposals = JavaTypeFinder
						.getClassProposals(resource, start, length);
				if (classProposals != null) {
					for (int j = 0; j < classProposals.length; j++) {
						...
						if (matchString.length() == 0
								|| proposal.getQualifiedName().toLowerCase()
										.startsWith(matchString.toLowerCase())
								|| proposal.getShortName().toLowerCase()
										.startsWith(matchString.toLowerCase())) {
				...
				IResource resource = getResource(contentAssistRequest);
				ICompletionProposal[] typeProposals = JavaTypeFinder
						.getTypeProposals(resource, start, length);
				...
				IResource resource = getResource(contentAssistRequest);
				ICompletionProposal[] beanNameProposals = JavaTypeFinder
						.getBeanProposals(resource, start, length);
