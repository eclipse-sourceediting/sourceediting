	...
	public String getPageLanguage(Node node) {
		...
		if (doc != null) {
			if (doc instanceof IDOMDocument) {
				PageDirectiveAdapter adapter = (PageDirectiveAdapter) ((IDOMDocument) doc)
				...
				NodeList pageDirectives = doc
						.getElementsByTagName(JSP12Namespace.ElementName.DIRECTIVE_PAGE);
				for (int i = 0; i < pageDirectives.getLength(); i++) {
					Element pageDirective = (Element) pageDirectives.item(i);
					String langValue = pageDirective
							.getAttribute(JSP11Namespace.ATTR_NAME_LANGUAGE);
