 ...
 * @plannedfor 1.0
 */
public class JavaTypeCompletionProposal extends CustomCompletionProposal
	...
	public JavaTypeCompletionProposal(String replacementString,
			int replacementOffset, int replacementLength, String qualifiedName,
			Image image, String typeName, String containerName, int relevence,
		...
		super(
				...
				image,
				(containerName != null && containerName.length() > 0) ? typeName
						+ " - " + containerName : typeName, null, null, relevence, true); //$NON-NLS-1$
		// CMVC 243817, superclass was comparing incorrect display string in
		// validate method...
		// super(replacementString, replacementOffset, replacementLength, image,
		// (containerName != null && containerName.length() > 0)? typeName + " -
		// " + containerName:typeName/*qualifiedName*/, relevence);
		fShortName = typeName;
	...
	protected String getImport(IStructuredDocumentRegion flatNode) {
		...
		for (int i = 0; i < regions.size(); i++) {
			...
			if (region.getType() == DOMRegionContext.XML_TAG_ATTRIBUTE_NAME) {
				if (flatNode.getText(region).equals(
						JSP11Namespace.ATTR_NAME_IMPORT)) {
					isImport = true;
				} else {
					isImport = false;
				}
	...
	protected int applyImport(IStructuredDocument model) {
		...
		if (fContainerName == null || fContainerName.length() == 0
				|| fContainerName.equals("java.lang")) {
			return 0;
		}
		// collect page directives and store their import values
		List imports = new ArrayList();
		IStructuredDocumentRegion node = model
				.getFirstStructuredDocumentRegion();

		// use the last position of a page directive as a hint as to where to
		// add
		// a new one
		int hint = 0;
		// watch for jsp:root so that we use the right XML/JSP format for the
		// directive
		boolean useXML = false;

		while (node != null) {
			// Can't just look for all StructuredDocumentRegions starting with
			// JSP_DIRECTIVE_OPEN
			...
			if (regions.size() > 1) {
				...
				if (name.getType() == DOMJSPRegionContexts.JSP_DIRECTIVE_NAME) {
					...
					if (node.getText(name)
							.equals(JSP11Namespace.ATTR_NAME_PAGE)
							|| node.getText(name).equals(
									JSP12Namespace.ElementName.DIRECTIVE_PAGE)) {
					...
					useXML = useXML
							|| name.getType() == DOMJSPRegionContexts.JSP_ROOT_TAG_NAME;
				}
			}
			node = node.getNext();
		}
	...
	/**
	 * See if the import specification is a wildcard import, and if so, that it
	 * applies to the given type.
	 */
	...
	public void apply(IDocument document, char trigger, int offset) {
		// If we have a parsed IStructuredDocument, insert the short name
		// instead of the
		...
		boolean addShortForm = false; // document instanceof
										// IStructuredDocument && fContainerName
										// != null && fContainerName.length() >
										// 0;
		if (addShortForm) {
	...
	protected boolean startsWith(IDocument document, int offset, String word) {
		...
		try {
			...
			// CMVC 243817
			// slightly modified to be a little more flexible for attribute
			// value string matching..
			String start = StringUtils.stripQuotes(document.get(
					getReplacementOffset(), length));
			return word.toLowerCase().startsWith(start.toLowerCase())
					|| fQualifiedName.toLowerCase().startsWith(
							start.toLowerCase());
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.wst.jsdt.internal.ui.text.java.JavaCompletionProposal#validate(org.eclipse.jface.text.IDocument,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.wst.jsdt.internal.ui.text.java.JavaCompletionProposal#selected(org.eclipse.jface.text.ITextViewer,
	 *      boolean)
	 */
	@Override
