...
/**
 * This class computes attribute value completion proposals for
 * &lt;jsp:[gs]etProperty&gt; tags.
 * 
 * @plannedfor 1.0
 */
public class JSPPropertyContentAssistProcessor extends
	...
	protected void addAttributeValueProposals(
		...
		// get the attribute in question (first attr name to the left of the
		// cursor)
		...
		for (int j = 0; j < openRegions.size(); j++) {
			...
			if (tagNameRegion.getType() == DOMRegionContext.XML_TAG_NAME
					&& open.getText(tagNameRegion).trim().equals(
							"jsp:setProperty")) { //$NON-NLS-1$
				isGetProperty = false;
				break;
			}
		}
		...
		// fixups since the matchString computations don't care if there are
		...
		if (currentValue.length() > StringUtils.strip(currentValue).length()
			...
			matchString = currentValue.substring(1, contentAssistRequest
					.getMatchString().length());
		} else {
			matchString = currentValue.substring(0, contentAssistRequest
					.getMatchString().length());
		}

		// for now we ignore complicated values such as jsp embedded in an
		// attribute
		...
		if (existingComplicatedValue) {
			...
			if (attributeName.equals(JSP11Namespace.ATTR_NAME_NAME)) {
				...
				addBeanPropertyProposals(contentAssistRequest, node,
						isGetProperty, matchString);
			}
		}
	}

	private void addBeanPropertyProposals(
			ContentAssistRequest contentAssistRequest, IDOMNode node,
			boolean isGetProperty, String matchString) {
		// assumes that the node is the [gs]etProperty tag
		String useBeanName = ((Element) node)
				.getAttribute(JSP11Namespace.ATTR_NAME_NAME);
		// properties can only be provided if a class/type/beanName has been
		// declared
		if (useBeanName != null && useBeanName.length() > 0) {
			NodeList useBeans = node.getOwnerDocument().getElementsByTagName(
					JSP11Namespace.ElementName.USEBEAN);
			if (useBeans != null) {
				...
				for (int j = 0; j < useBeans.getLength(); j++) {
					...
					if (useBean instanceof IndexedRegion
							&& ((IndexedRegion) useBean).getStartOffset() < node
									.getStartOffset()) {
						if (useBean.getAttribute(JSP11Namespace.ATTR_NAME_ID)
								.equals(useBeanName)) {
							typeName = useBean
									.getAttribute(JSP11Namespace.ATTR_NAME_CLASS);
							if (typeName == null || typeName.length() < 1) {
								typeName = useBean
										.getAttribute(JSP11Namespace.ATTR_NAME_TYPE);
							}
							if (typeName == null || typeName.length() < 1) {
								typeName = useBean
										.getAttribute(JSP11Namespace.ATTR_NAME_BEAN_NAME);
							}
						}
					}
				}
				if (typeName != null && typeName.length() > 0) {
					// find the class/type/beanName definition and obtain the
					// list of properties
					IBeanInfoProvider provider = new BeanInfoProvider();
					IResource resource = getResource(contentAssistRequest);
					IJavaPropertyDescriptor[] descriptors = provider
							.getRuntimeProperties(resource, typeName);
					CustomCompletionProposal proposal = null;
					String displayString = ""; //$NON-NLS-1$
					for (int j = 0; j < descriptors.length; j++) {
						IJavaPropertyDescriptor pd = descriptors[j];
						// check whether it's get or set kinda property
						if (pd.getReadable() && isGetProperty
								|| pd.getWriteable() && !isGetProperty) {
							// filter attr value name
							if (matchString.length() == 0
									|| pd.getName().toLowerCase().startsWith(
											matchString.toLowerCase())) {
								displayString = pd.getDisplayName();
								if (pd.getDeclaredType() != null
										&& pd.getDeclaredType().length() > 0) {
									displayString += " - " + pd.getDeclaredType(); //$NON-NLS-1$
								}
								proposal = new CustomCompletionProposal(
										"\"" + pd.getName() + "\"", //$NON-NLS-1$ //$NON-NLS-2$
	...
	private void addBeanNameProposals(
			ContentAssistRequest contentAssistRequest, IDOMNode node,
			String matchString) {
		...
		if (useBeans != null) {
			...
			for (int j = 0; j < useBeans.getLength(); j++) {
				...
				if (useBean instanceof IndexedRegion
						&& ((IndexedRegion) useBean).getStartOffset() < node
								.getStartOffset()) {
					id = StringUtils.strip(useBean
							.getAttribute(JSP11Namespace.ATTR_NAME_ID));
					displayString = null;
					classOrType = null;
					imageName = JSPEditorPluginImages.IMG_OBJ_CLASS_OBJ;
					// set the Image based on whether the class, type, or
					// beanName attribute is present
					classOrType = useBean
							.getAttribute(JSP11Namespace.ATTR_NAME_CLASS);
						...
						if (matchString.length() == 0
							...
							CustomCompletionProposal proposal = new CustomCompletionProposal(
									...
									id.length() + 2, JSPEditorPluginImageHelper
											.getInstance().getImage(imageName),
