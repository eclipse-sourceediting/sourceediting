...
/**
 * Use w/ JDT search engine to find all type names. Creates proposals from
 * matches reported.
 * 
 * @plannedfor 1.0
 */
public class JavaTypeNameRequestor extends TypeNameRequestor {
	...
	public void acceptType(int modifiers, char[] packageName,
		...
		JavaTypeCompletionProposal proposal = new JavaTypeCompletionProposal(
				fullName, offset, length, fullName, image, typeNameString,
				typeQualifier, IRelevanceConstants.R_NONE, true);
	...
	private Image calculateImage(int modifiers) {
		CompletionProposal p = CompletionProposal.create(
				CompletionProposal.TYPE_REF, getJSPOffset());
		...
		Image image = JSPEditorPluginImageHelper.getInstance().getImage(
				descriptor);
		return image;
	}
	...
	JavaTypeCompletionProposal[] getProposals() {
		return (JavaTypeCompletionProposal[]) fProposals
				.toArray(new JavaTypeCompletionProposal[fProposals.size()]);
	}
}
