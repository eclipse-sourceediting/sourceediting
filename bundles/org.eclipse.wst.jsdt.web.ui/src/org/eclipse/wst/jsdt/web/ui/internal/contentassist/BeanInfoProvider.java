...
/**
 * Navigates the IJavaProject classpath (incl. source) on a given resource and
 * infers bean properties given a fully qualified beanname. Bean properties can
 * be retrieved using:
...
public class BeanInfoProvider implements IBeanInfoProvider {
	...
	public class JavaPropertyDescriptor implements IJavaPropertyDescriptor {
		...
		public JavaPropertyDescriptor(String name, String type,
	...
	// looks up encoded type (see Class.getName), and gives you a displayable
	// string
	...
	/**
	 * Returns the inferred properties of a bean based on the project from the
	 * baseResource, and the fully qualified name of the bean.
	 * 
	 * @param baseResource
	 *            the base resource where the bean is being used
	 * @param typeName
	 *            the <i>fully qualified</i> type name (eg.
	 *            javax.swing.JButton) of the bean
	 */
	public IJavaPropertyDescriptor[] getRuntimeProperties(
			IResource baseResource, String typeName) {
		...
		try {
			IType type = javaProject.findType(typeQualifiedName.getQualifier()
					+ "." + typeQualifiedName.getLocalName()); //$NON-NLS-1$
			// type must exist
			if (type != null) {
				...
				for (int i = 0; i < methods.length; i++) {
					acceptMethod(getMethodResults, isMethodResults,
							setMethodResults, methods[i]);
				}
				...
				for (int i = 0; i < supers.length; i++) {
					...
					for (int j = 0; j < methods.length; j++) {
						acceptMethod(getMethodResults, isMethodResults,
								setMethodResults, methods[j]);
					}
				}
				adaptMethodsToPropertyDescriptors(getMethodResults,
						isMethodResults, setMethodResults, descriptorResults);
			}
		} catch (JavaModelException jmex) {
			Logger.logException(
					"Problem navigating JavaProject in BeanInfoProvider", jmex); //$NON-NLS-1$
		}

		IJavaPropertyDescriptor[] finalResults = new IJavaPropertyDescriptor[descriptorResults
				.size()];
		System.arraycopy(descriptorResults.toArray(), 0, finalResults, 0,
				descriptorResults.size());
		return finalResults;
	}

	/**
	 * Retrieves the necessary information from method declaration lists,
	 * creates and fills a list of JavaPropertyDescriptors.
	 * 
	 * @param getMethods
	 * @param isMethods
	 * @param setMethods
	 ...
	 */
	private void adaptMethodsToPropertyDescriptors(List getMethods,
			List isMethods, List setMethods, List descriptors)
			throws JavaModelException {
		List readable = new ArrayList();
		HashMap types = new HashMap();

		// iterate through get* and is* methods, updating 'readable' list and
		// 'types' map
		...
		while (it.hasNext()) {
			...
			if (readable.contains(name)) {
				...
				if (!fRepeatMethods.contains(name)) {
					descriptors.add(new JavaPropertyDescriptor(name,
							(String) types.get(name), true, true));
				...
				if (!fRepeatMethods.contains(name)) {
					...
					descriptors.add(new JavaPropertyDescriptor(name, type,
							false, true));
					fRepeatMethods.add(name);
				}
			}
		}
		// add leftover from readable, get* and is* methods (readable = true,
		// writable = false)
		it = readable.iterator();
		while (it.hasNext()) {
			name = (String) it.next();
			if (!fRepeatMethods.contains(name)) {
				descriptors.add(new JavaPropertyDescriptor(name, (String) types
						.get(name), true, false));
				fRepeatMethods.add(name);
			}
		}
	...
	private void filterGetMethods(List getMethods, List readable, HashMap types)
			throws JavaModelException {
		IMethod temp;
		String name;
	...
	private void filterIsMethods(List isMethodResults, List readable,
			HashMap types) throws JavaModelException {
		IMethod temp;
		String name;
		String encodedReturnType;
		String returnType;
	...
	/**
	 * Pass in a get*|set*|is* method and it will return an inferred property
	 * name using <code>Introspector.decapitalize(String)</code>
	 * 
	 * @param temp
	 * @return an inferred property name based on the IMethod name, null if the
	 *         name is not valid according to bean spec
	 */
	...
	/**
	 * Initial filtering of methods. Checks prefix if it's valid length. If the
	 * prefix is "get" the method name is placed in the getMethodResults List.
	 * If the prefix is "is", the name is added to the isMethodResults list. If
	 * the prefix is "set", it's added to the setMethodResultsList.
	 * 
	 ...
	 * @param method
	 */
	private void acceptMethod(List getMethodResults, List isMethodResults,
			List setMethodResults, IMethod method) throws JavaModelException {
	...
	/**
	 * @param typeName
	 * @return a Qualified name with the package as the qualifier, and class
	 *         name as LocalName
	 */
	...
	/**
	 * Returns the decoded (displayable) name fo the type. Either a primitive
	 * type (int, long, float...) Object (String)
	 * 
	 * @param type
	...
	private String getDecodedTypeName(String encoded) {
		...
		if (identifier == 'L' || identifier == 'Q') {
			// handle object
			String classname = encoded
					.substring(last + 1, encoded.length() - 1);
	...
	/**
	 * from Class.getName() javadoc also see Signature in jdt.core api
	 * 
	 * <pre>
	 * 			B            byte
	 * 			C            char
