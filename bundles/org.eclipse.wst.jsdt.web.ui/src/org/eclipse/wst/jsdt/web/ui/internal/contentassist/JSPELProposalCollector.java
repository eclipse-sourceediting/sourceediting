...
public class JSPELProposalCollector extends JSPProposalCollector {
	...
	public JSPELProposalCollector(ICompilationUnit cu,
	...
	protected IJavaCompletionProposal createJavaCompletionProposal(
		...
		if (proposal.getKind() == CompletionProposal.METHOD_REF
				&& proposal.findParameterNames(null).length == 0) {
			if (rawName.length() > 3 && rawName.startsWith("get")) { //$NON-NLS-1$
				completion = rawName.substring(3, 4).toLowerCase()
						+ rawName.substring(4, rawName.length());
			} else {
				return null;
			}
			...
			jspProposal = new JSPCompletionProposal(completion, offset, length,
					positionAfter, image, displayString, contextInformation,
					additionalInfo, relevance, updateLengthOnValidate);
	...
	// protected void acceptMethod(char[] declaringTypePackageName, char[]
	// declaringTypeName, char[] name, char[][] parameterPackageNames, char[][]
	// parameterTypeNames, char[][] parameterNames, char[]
	// returnTypePackageName, char[] returnTypeName, char[] completionName, int
	// modifiers, int start, int end, int relevance) {
	// String rawName = String.valueOf(name);
	// if(parameterNames.length == 0 && rawName.length() > 3 &&
	// rawName.startsWith("get"))
	// {
	// String mangledName = rawName.substring(3,4).toLowerCase() +
	// rawName.substring(4, rawName.length());
	// super.acceptField(declaringTypePackageName, declaringTypeName,
	// mangledName.toCharArray(), returnTypePackageName, returnTypeName,
	// mangledName.toCharArray(), modifiers, start, end, relevance);
	// }
	// }
}
