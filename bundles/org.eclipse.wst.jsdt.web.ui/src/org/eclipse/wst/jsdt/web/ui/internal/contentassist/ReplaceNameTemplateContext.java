 ...
 * during evaluation, the "prefix" before the template will be checked to see if
 * it matches the template name. If so, overwrite the template name. Otherwise,
 * just insert the template at the insert offset location (by not overwriting
 * the prefix text)
 */
public class ReplaceNameTemplateContext extends DocumentTemplateContext {
	 ...
	 */
	public ReplaceNameTemplateContext(TemplateContextType type,
	 ...
	 *            the length of the document region
	 * @param insertOffset
	 *            the offset of the document region where insert was originally
	 *            requested
	 */
	public ReplaceNameTemplateContext(TemplateContextType type,
			IDocument document, int offset, int length, int insertOffset) {
		this(type, document, new Position(offset, length));
	...
	public TemplateBuffer evaluate(Template template)
		...
		if (buffer != null) {
			if (fInsertOffset > -1 && fInsertOffset > getStart()) {
				String prefix = getDocument().get(getStart(),
						fInsertOffset - getStart());
				if (!template.getName().startsWith(prefix)) {
					// generate a new buffer that actually contains the
					// text that was going to be overwritten
					buffer = new TemplateBuffer(prefix + buffer.getString(),
							buffer.getVariables());
				}
			}
		}
