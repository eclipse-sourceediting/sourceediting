...
public class JSPJavaContentAssistProcessor implements IContentAssistProcessor,
	...
	public ICompletionProposal[] computeCompletionProposals(ITextViewer viewer,
		...
		IndexedRegion treeNode = ContentAssistUtils.getNodeAt(viewer,
				documentPosition);
		...
		if (treeNode instanceof IDOMNode) {
			...
			if (flat != null
					&& flat.getType() == DOMJSPRegionContexts.JSP_CONTENT) {
		...
		// if (flat != null && flat.getType() !=
		// DOMJSPRegionContexts.JSP_SCRIPTLET_OPEN && flat.getType() !=
		// DOMJSPRegionContexts.JSP_DECLARATION_OPEN && flat.getType() !=
		// DOMJSPRegionContexts.JSP_EXPRESSION_OPEN && flat.getType() !=
		// DOMRegionContext.BLOCK_TEXT && (openRegion != null &&
		...
		// }
		//
		// CustomCompletionProposal cdataProposal =
		// createCDATAProposal(adjustedDocPosition, selectionLength);
		// ICompletionProposal[] newResults = new
		// ICompletionProposal[results.length + 1];
		...
		// if(tagText.equals("<%") || tagText.equals("<%=") ||
		// tagText.equals("<%!"));
		// {
		// ICompletionProposal testah =
		// ContentAssistUtils.computeJSPEndTagProposal(viewer,documentPosition,
		// treeNode, "<%" ,
	...
	public IContextInformation[] computeContextInformation(ITextViewer viewer,
		...
		if (dp != null) {
			...
			// viewer.getDocument().getDocumentPartitioner();
			String type = dp.getPartition(documentOffset).getType();
			if (type == IJSPPartitions.JSP_DEFAULT
					|| type == IJSPPartitions.JSP_CONTENT_JAVA) {
				...
				for (int i = 0; i < proposals.length; i++) {
					IContextInformation ci = proposals[i]
		...
		return (IContextInformation[]) results
				.toArray(new IContextInformation[results.size()]);
	}
