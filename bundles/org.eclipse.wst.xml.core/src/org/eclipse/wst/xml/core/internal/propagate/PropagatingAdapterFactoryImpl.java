...
/**
 * The PropagatingAdapterFactory is part of the "adapt on create"
 * mechanism. A PropagatingAdapter, once added to a node, will 
 * cause proagating adapters to be created for all child nodes.
 * A side effect of creating a PropagatingAdapter for a node 
 * is that is is also creates adapters for and adapts the Node for 
 * all other registered 'create on adapt' Adapters. This other
 * adapters are registered by registering their factories 
 * via plugin extension point. 
 */
public class PropagatingAdapterFactoryImpl extends AbstractAdapterFactory implements PropagatingAdapterFactory {
	...
	protected PropagatingAdapterFactoryImpl(Object adapterKey, boolean registerAdapters) { //, Object modelType)  {
