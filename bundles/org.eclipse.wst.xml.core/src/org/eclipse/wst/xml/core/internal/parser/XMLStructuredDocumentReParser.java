...
public class XMLStructuredDocumentReParser extends StructuredDocumentReParser {
	...
	/**
	 * The rule is, that is we are content, preceded by lone <, then
	 * we need to advance one more for dirty end.
	 */
	...
	protected IStructuredDocumentRegion findDirtyEnd(int end) {
		...
		// 12/6/2001 - Since we've changed the parser/scanner to allow a lone '<' without
		// always interpretting it as start of a tag name, we need to be a little fancier, in order
		// to "skip" over any plain 'ol content between the lone '<' and any potential meating
	...
	protected void findDirtyStart(int start) {
		...
		// go back one more, if it exists.  This prevents problems with insertions
		// of text that should be merged with the previous node instead of simply hung
		// off of it as a separate node (ex.: XML content inserted right before an open 
		...
		if (result != null) {
			...
			// If we are now at the end of a "tag dependent" content area (or JSP area)
