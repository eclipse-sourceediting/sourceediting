...
public class ContextRegionContainer implements ITextRegionContainer {
	...
	public String getFullText(org.eclipse.wst.sse.core.text.ITextRegion aRegion) {
		// Must be proxied here since aRegion should always be a child of *this* container and indexed from
	...
	public int getTextEndOffset() {
		...
		// TODO CRITICAL -- need to re-work this work around, so doesn't depend on XMLRegionContext
		//		// this is a workaround for 226823///////////
		//		for (int i = regions.size() - 1; i >= 0 && region.getType() == XMLRegionContext.WHITE_SPACE; i--)
	...
	/**
	 * same as getFullText for this region type ... do we need
	 * to take white space off?
	 */
	...
	/**
	 * This method is just to equate positions.
	 * clients may (will probably) still need to
	 * make calls to equate regions, parent, etc. 
	 */
	...
	public int getTextEnd() {
		//		int result = 0;
		//		ITextRegion lastRegion = (ITextRegion) regions.get(regions.size() - 1);
	...
	public StructuredDocumentEvent updateModel(Object requester, IStructuredDocumentRegion parent, String changes, int requestStart, int lengthToReplace) {
		...
		// FUTURE_TO_DO: need to implement region level parsing in ITextRegionContainer::updateModel
		// never being called?
		return result;
	}
