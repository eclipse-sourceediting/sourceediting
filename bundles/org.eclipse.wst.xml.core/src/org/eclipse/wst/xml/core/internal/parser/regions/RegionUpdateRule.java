...
/**
 * 
 * This is a utility class to centralize 'region' update. 
 * Note: care must be taken that is is not used for StructuredDocumentRegions, 
 * or container regions, its only for "token regions"
 * 
 */
public class RegionUpdateRule {
	...
	static public StructuredDocumentEvent updateModel(ITextRegion region, Object requester, IStructuredDocumentRegion parent, String changes, int requestStart, int lengthToReplace) {
		...
		// (This can obviously be made more sophisticated as the need arises, but should
		...
		if (canHandle) {
			...
			// and in turn, the structuredDocument itself has responsibility for
			...
			// that is in only the trailing whitespace region, we should not do a quick change,
			...
			// If we are handling as whitespace, there is no need to increase the text length, only
	...
	static public boolean canHandleAsWhiteSpace(ITextRegion region, IStructuredDocumentRegion parent, String changes, int requestStart, int lengthToReplace) {
		...
		// (in which case, we must be dealing with regions that are structuredDocumentRegions).
		if (parent == null)
