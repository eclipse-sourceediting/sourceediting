...
public class XMLModelNotifierImpl implements XMLModelNotifier {
	...
	private void notifyDeferred() {
		...
		for (int i = 0; i < count; i++) {
			...
			if (!doingNewModel && fOptimizeDeferred) {
				// check redundant events (no need to check if doing NewModel, since
	...
	private void setCommonRootIfNeeded(Node node) {
		...
		if (this.changedRoot == null) {
			...
			// tiny optimization: if previous commonAncestor (changedRoot) is already 'document', 
	...
	public void valueChanged(Node node) {
		...
		if (node.getNodeType() == Node.ATTRIBUTE_NODE) {
			...
			// TODO_dmw: experimental: changed 06/29/2004 to send "strucuture changed" even for attribute value changes
			// there are pros and cons to considering attribute value "structure changed". Will (re)consider
			...
			// note: we do not send structured changed event for content changed
