...
public class XMLModelParser implements org.eclipse.wst.xml.core.jsp.model.parser.temp.XMLJSPRegionContexts {
	...
	private void changeTagName(IStructuredDocumentRegion flatNode, ITextRegion region) {
		...
		// do nothing, but...
		// if it's not a change in the end tag of an element with the start tag,
	...
	/**
	 * This routine create an Element from comment data for comment style elements,
	 * such as SSI and METADATA
	 */
		...
		if (next != null) {
			...
			if (nodeType != Node.ELEMENT_NODE) {
				...
				if (text.getStructuredDocumentRegion() == null) {
					// this is the case partial IStructuredDocumentRegion is removed
					removeNode(text);
				}
				else {
					// notify the change
				...
				// if shared, continue to remove IStructuredDocumentRegion from them
				if (isShared)
					removeStructuredDocumentRegion(oldStructuredDocumentRegion);
				return;
			}
	...
	/**
	 * Remove the specified node if it is no longer required implicit tag
	 * with remaining child nodes promoted.
	 */
	...
	private void removeStartTag(Element element) {
		...
		Node startElement = null; // for the case element is removed as end tag
		if (oldElement.hasEndTag()) {
