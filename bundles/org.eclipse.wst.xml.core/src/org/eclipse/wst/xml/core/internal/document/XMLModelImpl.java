...
public class XMLModelImpl extends AbstractStructuredModel implements IStructuredDocumentListener, XMLModel, DOMImplementation {
	private String TRACE_PARSER_MANAGEMENT_EXCEPTION = "parserManagement"; //$NON-NLS-1$
	 ...
	 *
	 * Note that the client who is making these changes has the
	 * responsibility to restore the models state once finished
	 * with the changes. See getMemento and restoreState.
	 *
	 * The method isModelStateChanging can be used by a client
	 * to determine if the model is already in a change sequence.
	 */
	...
	public void changedModel() {
		...
		// By calling changedModel first, this basically decrements the "isChanging" counter
		// in super class and when zero all listeners to model state events will be notified
		// that the model has been changed. 'endChanging' will notify all deferred adapters. 
		// So, the significance of order is that adapters (and methods they call)
		// can count on the state of model "isChanging" to be accurate. 
		// But, remember, that this means the "modelChanged" event can be received before all 
	...
	public void setStructuredDocument(IStructuredDocument structuredDocument) {
		...
		if (structuredDocument != null)
			structuredDocument.addModelChangingListener(this);
	}
