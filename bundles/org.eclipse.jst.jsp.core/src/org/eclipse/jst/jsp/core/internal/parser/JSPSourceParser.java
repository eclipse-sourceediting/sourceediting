	...
	protected IStructuredDocumentRegion parseNodes() {
		...
		try {
			...
			while ((region = getNextRegion()) != null) {
				type = region.getType();
				// these types (might) demand a IStructuredDocumentRegion for
				// each
				// of them
				if (type == DOMRegionContext.BLOCK_TEXT) {
					if (currentNode != null && currentNode.getLastRegion().getType() == DOMRegionContext.BLOCK_TEXT) {
						// multiple block texts indicated embedded containers;
						// no
				...
				// this is extremely rare, but valid
					...
					if (lastRegion instanceof ITextRegionContainer) {
						...
						// DW, 4/16/2003 container regions have parent.
						// Probably a
						// better place to set,
				...
				// these regions also get their own node, so close them
				// cleanly
				// NOTE: these regions have new StructuredDocumentRegions
				// created
				// for them above; it may
				// be more readable if that is handled here as well, but the
				// current layout
				// ensures that they open StructuredDocumentRegions the same
				// way
			...
			Logger.log(Logger.ERROR, "Parser error: " + e.getMessage());
		}
		return headNode;
	}
