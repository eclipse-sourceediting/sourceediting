...
public class XSLValidator {
	...
	public ValidationReport validate(String uri, InputStream inputStream,
		...
		if (configuration == null) {
			configuration = new XSLValidationConfiguration();
		}
		...
		try {
			...
			grammarPreparser.registerPreparser(
					XMLGrammarDescription.XML_SCHEMA, null/* schemaLoader */);

			grammarPreparser.setProperty(Constants.XERCES_PROPERTY_PREFIX
					+ Constants.XMLGRAMMAR_POOL_PROPERTY,
					new XMLGrammarPoolImpl());
			grammarPreparser.setFeature(Constants.XERCES_FEATURE_PREFIX
					+ Constants.CONTINUE_AFTER_FATAL_ERROR_FEATURE, false);
			grammarPreparser.setFeature(Constants.XERCES_FEATURE_PREFIX
			...
			grammarPreparser.setFeature(Constants.XERCES_FEATURE_PREFIX
					+ Constants.EXTERNAL_GENERAL_ENTITIES_FEATURE, true);
			grammarPreparser.setFeature(Constants.XERCES_FEATURE_PREFIX
					+ Constants.EXTERNAL_PARAMETER_ENTITIES_FEATURE, true);
			grammarPreparser.setFeature(Constants.XERCES_FEATURE_PREFIX
					+ Constants.WARN_ON_DUPLICATE_ATTDEF_FEATURE, true);

			if (configuration
					.getFeature(XSLValidationConfiguration.HONOUR_ALL_SCHEMA_LOCATIONS)) {
				try {
					grammarPreparser.setFeature(Constants.XERCES_FEATURE_PREFIX
							+ "honour-all-schemaLocations", true); //$NON-NLS-1$
				} catch (Exception e) {
			...
			if (configuration
					.getFeature(XSLValidationConfiguration.FULL_SCHEMA_CONFORMANCE)) {
				try {
					grammarPreparser.setFeature(Constants.XERCES_FEATURE_PREFIX
			...
			if (uriresolver != null) {
				XSDEntityResolver resolver = new XSDEntityResolver(uriresolver,
						uri);
				if (resolver != null) {
			...
			try {
				XMLInputSource is = new XMLInputSource(null, uri, uri,
						inputStream, null);
				grammarPreparser.getLoader(XMLGrammarDescription.XML_SCHEMA);
				grammarPreparser.preparseGrammar(
						XMLGrammarDescription.XML_SCHEMA, is);
			} catch (Exception e) {
	...
	/**
	 * The XSDErrorHandler handle Xerces parsing errors and puts the errors into
	 * the given ValidationInfo object.
	 */
	protected class XSDErrorHandler implements XMLErrorHandler {
		 ...
		 */
		protected void addValidationMessage(String errorKey,
				XMLParseException exception, int severity) {
			String systemId = exception.getExpandedSystemId();
			if (systemId != null) {
				if (severity == DOMError.SEVERITY_WARNING) {
					valinfo.addWarning(exception.getLocalizedMessage(),
							exception.getLineNumber(), exception
									.getColumnNumber(), systemId);
				} else {
					valinfo.addError(exception.getLocalizedMessage(), exception
							.getLineNumber(), exception.getColumnNumber(),
							systemId, errorKey, null);
				}
			}
		}
	...
	protected class XSDEntityResolver implements XMLEntityResolver {
		...
		public XMLInputSource resolveEntity(
			...
			// file I/O problems produced messages. I've remove this fudge for
			// now
			// since I can't seem to reproduce the problem it was intended to
			// fix.
			// I'm hoping the newer Xerces code base has fixed this problem and
			// the fudge is defunct.
			return XMLValidator._internalResolveEntity(uriresolver,
					resourceIdentifier);
