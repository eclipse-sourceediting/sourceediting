 ...
 */
/*
 * $Id: CountersTable.java,v 1.2 2008/03/28 02:38:16 dacarver Exp $
 */
...
public class CountersTable extends Hashtable {
	 ...
	 *            the given xsl:number element.
	 * 
	 * @return an empty vector to be used to store counts for this number
	 *         element.
	 */
	...
	/**
	 * Add a list of counted nodes that were built in backwards document order,
	 * or a list of counted nodes that are in forwards document order.
	 * 
	 * @param flist
	...
	/**
	 * Count forward until the given node is found, or until we have looked to
	 * the given amount.
	 * 
	 * @param support
	...
	public int countNode(XPathContext support, ElemNumber numberElem, int node)
		...
		if (DTM.NULL != target) {
			...
			// In the loop below, we collect the nodes in backwards doc order,
			// so
			...
			// of forward counting by one, this will mean a single node copy
			// from
			...
			for (; DTM.NULL != target; target = numberElem.getPreviousNode(
					support, target)) {

				// First time in, we should not have to check for previous
				// counts,
			...
			Counter counter = new Counter(numberElem, new NodeSetDTM(support
					.getDTMManager()));
