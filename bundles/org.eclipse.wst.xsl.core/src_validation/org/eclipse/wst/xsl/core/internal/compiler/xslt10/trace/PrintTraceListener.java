 ...
 */
/*
 * $Id: PrintTraceListener.java,v 1.2 2008/03/28 02:38:17 dacarver Exp $
 */
	...
	public void selected(SelectionEvent ev)
		...
		if (m_traceSelection) {
			...
			if (sourceNode instanceof DTMNodeProxy) {
				int nodeHandler = ((DTMNodeProxy) sourceNode)
						.getDTMNodeNumber();
				locator = ((DTMNodeProxy) sourceNode).getDTM()
						.getSourceLocatorFor(nodeHandler);
			}

			if (locator != null)
				m_pw.println("Selected source node '"
						+ sourceNode.getNodeName() + "', at " + locator);
			else
				m_pw.println("Selected source node '"
			...
			if (ev.m_styleNode.getLineNumber() == 0) {
				...
				} else if (parent == ete.getStylesheetRoot()
						.getDefaultTextRule()) {
			...
			if (ev.m_selection.getType() == XObject.CLASS_NODESET) {
				...
				try {
					...
					m_pw
							.println("     [Can't trace nodelist because it it threw a CloneNotSupportedException]");
					return;
				}
	...
	public void generated(GenerateEvent ev) {
		...
		if (m_traceGeneration) {
			...
			case SerializerTrace.EVENTTYPE_CHARACTERS: {
				String chars = new String(ev.m_characters, ev.m_start,
						ev.m_length);
	...
	public void extension(ExtensionEvent ev) {
		if (m_traceExtension) {
			...
			case ExtensionEvent.DEFAULT_CONSTRUCTOR:
				m_pw.println("EXTENSION: " + ((Class) ev.m_method).getName()
						+ "#<init>");
				break;
			case ExtensionEvent.METHOD:
				m_pw.println("EXTENSION: "
						+ ((Method) ev.m_method).getDeclaringClass().getName()
						+ "#" + ((Method) ev.m_method).getName());
				break;
			case ExtensionEvent.CONSTRUCTOR:
				m_pw.println("EXTENSION: "
						+ ((Constructor) ev.m_method).getDeclaringClass()
								.getName() + "#<init>");
				break;
			}
		}
