 ...
 */
/*
 * $Id: ClonerToResultTree.java,v 1.2 2008/03/28 02:38:16 dacarver Exp $
 */
...
public class ClonerToResultTree {
	...
	public static void cloneToResultTree(int node, int nodeType, DTM dtm,
			SerializationHandler rth, boolean shouldCloneAttributes)
			throws TransformerException {
		...
		try {
			...
			case DTM.ELEMENT_NODE: {
				...
				// call a SAX-like startElement, to be able to add attributes
				// after this call
				rth.startElement(ns, localName, dtm.getNodeNameX(node));
			...
			case DTM.NAMESPACE_NODE:
				// %REVIEW% Normally, these should have been handled with
				// element.
				// It's possible that someone may write a stylesheet that tries
				// to
				// clone them explicitly. If so, we need the equivalent of
				// rth.addAttribute().
				SerializerUtils.processNSDecls(rth, node, DTM.NAMESPACE_NODE,
						dtm);
				break;
			case DTM.COMMENT_NODE:
				XMLString xstr = dtm.getStringValue(node);
			...
			case DTM.PROCESSING_INSTRUCTION_NODE: {
				// %REVIEW% Is the node name the same as the "target"?
				rth.processingInstruction(dtm.getNodeNameX(node), dtm
						.getNodeValue(node));
			}
				break;
			default:
				// "Can not create item in result tree: "+node.getNodeName());
				throw new TransformerException("Can't clone node: "
						+ dtm.getNodeName(node));
			}
		} catch (org.xml.sax.SAXException se) {
			throw new TransformerException(se);
		}
