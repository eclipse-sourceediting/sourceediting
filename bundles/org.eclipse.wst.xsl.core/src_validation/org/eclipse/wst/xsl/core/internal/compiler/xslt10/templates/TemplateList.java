 ...
 */
/*
 * $Id: TemplateList.java,v 1.4 2008/03/28 02:38:15 dacarver Exp $
 */
...
public class TemplateList implements java.io.Serializable {
	 ...
	 * templates with match patterns. This routine should be called in
	 * decreasing order of precedence but it checks nonetheless.
	 * 
	 * @param template
	 */
	public void setTemplate(ElemTemplate template) {
		...
		if (null != template.getName()) {
			ElemTemplate existingTemplate = (ElemTemplate) m_namedTemplates
					.get(template.getName());
			if (null == existingTemplate) {
				m_namedTemplates.put(template.getName(), template);
			} else {
				int existingPrecedence = existingTemplate
						.getStylesheetComposed().getImportCountComposed();
				int newPrecedence = template.getStylesheetComposed()
	...
	void dumpAssociationTables() {
		...
		while (associations.hasMoreElements()) {
			TemplateSubPatternAssociation head = (TemplateSubPatternAssociation) associations
					.nextElement();
	...
	public void compose(StylesheetRoot sroot) {
		...
		if (null != m_wildCardPatterns) {
			...
			while (associations.hasMoreElements()) {
				...
				while (null != wild) {
					try {
						head = insertAssociationIntoList(head,
								(TemplateSubPatternAssociation) wild.clone(),
								true);
	 ...
	 *            <code>true</code> if we are inserting a wild card template
	 *            onto this list.
	 * @return the new head of the list.
	 */
	private TemplateSubPatternAssociation insertAssociationIntoList(
		...
		boolean insertBefore; // true means insert before insertPoint;
								// otherwise after
		...
		// with
		// a precedence that is either (i) less than ours or (ii) the same as
		// ours but
		// the template document position is less than ours
		// -or-
		// (b) the last template on the chain if no such template described in
		// (a) exists.
		// If we are pointing to the first template or the last template (that
		// is, case b),
		...
		while (true) {
			...
			if (null == next)
				...
				if (importLevel > next.getImportLevel())
					...
					insertPoint = next;
				else if (priority > workPriority) // import precedence is
													// equal
					break;
		...
		if ((null == next) || (insertPoint == head)) // insert point is first
														// or last
		{
	...
	private void insertPatternInTable(StepPattern pattern, ElemTemplate template) {
		...
		if (null != target) {
			...
			TemplateSubPatternAssociation association = new TemplateSubPatternAssociation(
					template, pattern, pstring);

			// See if there's already one there
			boolean isWildCard = association.isWild();
			TemplateSubPatternAssociation head = isWildCard ? m_wildCardPatterns
					: getHead(target);
	 ...
	 *         pattern to template associations.
	 */
	public TemplateSubPatternAssociation getHead(XPathContext xctxt,
		...
		case DTM.ATTRIBUTE_NODE:
			head = (TemplateSubPatternAssociation) m_patternTable.get(dtm
					.getLocalName(targetNode));
			break;
		case DTM.TEXT_NODE:
		...
		case DTM.ENTITY_NODE:
			head = (TemplateSubPatternAssociation) m_patternTable.get(dtm
					.getNodeName(targetNode)); // %REVIEW% I think this is
												// right
			break;
	 ...
	 */
	public ElemTemplate getTemplateFast(XPathContext xctxt, int targetNode,
			int expTypeID, QName mode, int maxImportLevel,
		...
		case DTM.ATTRIBUTE_NODE:
			head = (TemplateSubPatternAssociation) m_patternTable.get(dtm
					.getLocalNameFromExpandedNameID(expTypeID));
			break;
		case DTM.TEXT_NODE:
		case DTM.CDATA_SECTION_NODE:
			head = m_textPatterns;
		...
		try {
			do {
				if ((maxImportLevel > -1)
						&& (head.getImportLevel() > maxImportLevel)) {
					continue;
				}
				ElemTemplate template = head.getTemplate();
				xctxt.setNamespaceContext(template);

				if ((head.m_stepPattern.execute(xctxt, targetNode, dtm,
						expTypeID) != NodeTest.SCORE_NONE)
						&& head.matchMode(mode)) {
	 ...
	 * @throws TransformerException
	 */
	public ElemTemplate getTemplate(XPathContext xctxt, int targetNode,
	 ...
	 * linked lists of the actual patterns that match the target element to some
	 * degree of specifity.
	 * 
	 * @serial
	 */
	 ...
	 *         as the keys, and {@link org.apache.xalan.templates.ElemTemplate}s
	 *         as the values.
	 */
	 ...
	 * 
	 * @param v
	 *            Hashtable dictionary that contains {@link java.lang.String}s
	 *            as the keys, and
	 *            {@link org.apache.xalan.templates.ElemTemplate}s as the
	 *            values.
	 */
	 ...
	 * 
	 * @return The head of a linked list that contains all possible match
	 *         pattern to template associations for the given key.
	 */
	...
	/**
	 * An inner class used by a compiler to iterate over all of the
	 * ElemTemplates stored in this TemplateList. The compiler can replace
	 * returned templates with their compiled equivalent.
	 */
	public class TemplateWalker {
		...
		public ElemTemplate next() {
			...
			while (true) {
				if (inPatterns) {
					...
					if (null != curPattern)
						...
						if (hashIterator.hasMoreElements()) {
							curPattern = (TemplateSubPatternAssociation) hashIterator
									.nextElement();
							retValue = curPattern.getTemplate();
						} else {
							inPatterns = false;
				...
				ct = (ElemTemplate) m_compilerCache.get(new Integer(retValue
						.getUid()));
				if (null == ct) {
					m_compilerCache.put(new Integer(retValue.getUid()),
							retValue);
					return retValue;
				}
			}
