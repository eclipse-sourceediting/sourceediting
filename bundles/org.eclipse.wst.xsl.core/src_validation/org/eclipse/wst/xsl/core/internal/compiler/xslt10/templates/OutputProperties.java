 ...
 */
/*
 * $Id: OutputProperties.java,v 1.3 2008/03/28 02:38:15 dacarver Exp $
 */
 ...
 * {@link org.apache.xalan.templates.ElemTemplateElement} heararchy.
 * <p>
...
public class OutputProperties extends ElemTemplateElement implements Cloneable {
	...
	/**
	 * Creates an empty OutputProperties with the defaults specified by a
	 * property file. The method argument is used to construct a string of the
	 * form output_[method].properties (for instance, output_html.properties).
	 * The output_xml.properties file is always used as the base.
	 * <p>
	 * At the moment, anything other than 'text', 'xml', and 'html', will use
	 * the output_xml.properties file.
	 * </p>
	 * 
	 * @param method
	...
	public OutputProperties(String method) {
		m_properties = new Properties(OutputPropertiesFactory
	...
	public void setProperty(String key, String value) {
		...
		if (key
				.startsWith(OutputPropertiesFactory.S_BUILTIN_OLD_EXTENSIONS_UNIVERSAL))
			key = OutputPropertiesFactory.S_BUILTIN_EXTENSIONS_UNIVERSAL
					+ key
							.substring(OutputPropertiesFactory.S_BUILTIN_OLD_EXTENSIONS_UNIVERSAL_LEN);
	 ...
	 * the key is not found in this property list, the default property list,
	 ...
	 * <code>false</code> if the property is not found, or if the value is
	 * other than "yes".
	 * 
	 * @param key
	 *            the property key.
	 ...
	 * @param key
	 *            the property key.
	 * @return the value in this property list as a int value, or false if null
	 *         or not a number.
	 */
	...
	/**
	 * Set an output property with a QName value. The QName will be turned into
	 * a string with the namespace in curly brackets.
	 * 
	 * @param key
	 *            the key to be placed into the property list.
	 * @param value
	...
	public void setMethodDefaults(String method) {
		...
		// bjm - add the next condition as a hack
			...
			Properties newDefaults = OutputPropertiesFactory
					.getDefaultMethodProperties(method);
	 ...
	 * @param key
	 *            the property key.
	 * @return the value in this property list as a QName value, or false if
	 *         null or not "yes".
	 */
		...
		if (null != s) {
			...
			for (int i = 0; i < l; i++) {
				...
				if (Character.isWhitespace(c)) {
					if (!inCurly) {
						if (buf.length() > 0) {
							QName qname = QName.getQNameFromString(buf
									.toString());
							v.addElement(qname);
							buf.reset();
						}
	...
	/**
	 * Copy the keys and values from the source to this object. This will not
	 * copy the default values. This is meant to be used by going from a higher
	 ...
	 *            true if the defaults should be reset based on the method
	 *            property.
	 */
		...
		while (keys.hasMoreElements()) {
			...
			if (!isLegalPropertyKey(key))
				throw new IllegalArgumentException(Messages.createMessage(
						...
						new Object[] { key })); // "output property not
												// recognized: "
			...
			if (null == oldValue) {
				...
				m_properties.put(key, (String) oldValue + " "
						+ (String) src.get(key));
			}
		}
	}
	...
	public static boolean isLegalPropertyKey(String key) {
		...
		return (key.equals(OutputKeys.CDATA_SECTION_ELEMENTS)
				...
				|| key.equals(OutputKeys.VERSION) || (key.length() > 0)
				&& (key.charAt(0) == '{') && (key.lastIndexOf('{') == 0)
				&& (key.indexOf('}') > 0)
