 ...
 */
/*
 * $Id: ProcessorLRE.java,v 1.3 2008/03/28 02:38:16 dacarver Exp $
 */
 ...
 * The actual
 * {@link org.eclipse.wst.xsl.core.internal.compiler.xslt10.templates.ElemTemplateElement}
 * produced may be a
 * {@link org.eclipse.wst.xsl.core.internal.compiler.xslt10.templates.ElemLiteralResult},
 * a
 * {@link org.eclipse.wst.xsl.core.internal.compiler.xslt10.templates.StylesheetRoot},
 * or a
...
public class ProcessorLRE extends ProcessorTemplateElem {
	...
	public void startElement(StylesheetHandler handler, String uri,
		...
		try {
			...
			if (null == p) {
				...
				XSLTElementProcessor stylesheetProcessor = handler
						.getProcessorFor(
				...
				for (int i = 0; i < n; i++) {
					...
					if ((null != attrUri)
						...
						stylesheetAttrs.addAttribute(null, attrLocalName,
								attrLocalName, attributes.getType(i),
								attributes.getValue(i));
					} else if ((attrLocalName.startsWith("xmlns:") || attrLocalName
							.equals("xmlns"))
							&& value
									.equals(org.apache.xml.utils.Constants.S_XSLNAMESPACEURL)) {
						...
						lreAttrs.addAttribute(attrUri, attrLocalName,
								attributes.getQName(i), attributes.getType(i),
								attributes.getValue(i));
					}
				}
				...
				try {
					stylesheetProcessor.setPropertiesFromAttributes(handler,
							"stylesheet", stylesheetAttrs, stylesheet);
				} catch (Exception e) {
				...
				XPath rootMatch = new XPath("/", stylesheet, stylesheet,
						XPath.MATCH, handler.getStylesheetProcessor()
								.getErrorListener());
			...
			while (null != p) {
				...
				if (p instanceof ElemLiteralResult) {
					...
					if ((false == isExtension)
							&& (null != uri)
							&& (uri
									.equals(org.apache.xml.utils.Constants.S_BUILTIN_EXTENSIONS_URL) || uri
			...
			try {
				...
				if (elem instanceof ElemLiteralResult) {
					...
					((ElemLiteralResult) elem)
							.setIsLiteralResultAsStylesheet(isLREAsStyleSheet);
				}
			} catch (InstantiationException ie) {
				handler.error(
						XSLTErrorResources.ER_FAILED_CREATING_ELEMLITRSLT,
						null, ie);// "Failed creating ElemLiteralResult
									// instance!", ie);
			} catch (IllegalAccessException iae) {
				handler.error(
						XSLTErrorResources.ER_FAILED_CREATING_ELEMLITRSLT,
						null, iae);// "Failed creating ElemLiteralResult
									// instance!", iae);
			}
			...
			if (!isExtension && (elem instanceof ElemLiteralResult)) {
				isExtension = ((ElemLiteralResult) elem)
						.containsExtensionElementURI(uri);
	...
	protected Stylesheet getStylesheetRoot(StylesheetHandler handler)
		...
		stylesheet = new StylesheetRoot(handler.getSchema(), handler
				.getStylesheetProcessor().getErrorListener());
