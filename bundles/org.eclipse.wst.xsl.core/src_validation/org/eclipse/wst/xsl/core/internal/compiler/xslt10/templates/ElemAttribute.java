 ...
 */
/*
 * $Id: ElemAttribute.java,v 1.3 2008/03/28 02:38:15 dacarver Exp $
 */
...
public class ElemAttribute extends ElemElement {
		...
		if (null != prefix && (prefix.length() == 0 || prefix.equals("xmlns"))) {
			...
			if (null == prefix || prefix.length() == 0
				...
				if (nodeNamespace.length() > 0) {
					NamespaceMappings prefixMapping = rhandler
	...
	void constructNode(String nodeName, String prefix, String nodeNamespace,
		...
		if (null != nodeName && nodeName.length() > 0) {
			...
			try {
				// Let the result tree handler add the attribute and its String
				// value.
				String localName = QName.getLocalPart(nodeName);
				if (prefix != null && prefix.length() > 0) {
					rhandler.addAttribute(nodeNamespace, localName, nodeName,
							"CDATA", val, true);
				} else {
					rhandler.addAttribute("", localName, nodeName, "CDATA",
							val, true);
				}
