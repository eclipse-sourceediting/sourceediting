 ...
 */
/*
 * $Id: StylesheetComposed.java,v 1.2 2008/03/28 02:38:15 dacarver Exp $
 */
 ...
 * <li>Properties that aren't found, will be searched for first in the
 * includes, and, if none are located, will be searched for in the imports.</li>
...
public class StylesheetComposed extends Stylesheet {
	 ...
	 * recomposableElements Vector that was passed in as the first parameter.
	 * All elements added to the recomposableElements vector should extend
	 * ElemTemplateElement.
	 * 
	 * @param recomposableElements
	 *            a Vector of ElemTemplateElement objects that we will add all
	 *            of our recomposable objects to.
	 */
	...
	void recomposeImports() {
		...
		if (count > 0) {
			...
			while (count > 0)
				m_endImportCountComposed += this.getImport(--count)
		...
		while (count > 0) {
			...
			while (imports > 0)
				m_endImportCountComposed += getIncludeComposed(count)
						.getImport(--imports).getEndImportCountComposed();
	...
	/**
	 * Get the precedence of this stylesheet in the global import list. The
	 * lowest precedence is 0. A higher number has a higher precedence.
	 * 
	 * @see <a href="http://www.w3.org/TR/xslt#import">import in XSLT
	 *      Specification</a>
	 * 
	 ...
	 * appending to) previous composition. We could phase out the old API in
	 * favor of this one, but I'm holding off until we've made up our minds
	 * about compilation. ADDED 9/5/2000 to support compilation experiment.
	 * NOTE: GLP 29-Nov-00 I've left this method in so that
	 * CompilingStylesheetHandler will compile. However, I'm not sure why it's
	 * needed or what it does and I've commented out the body.
	 * 
	 * @see <a
	 *      href="http://www.w3.org/TR/xslt#section-Defining-Template-Rules">section-Defining-Template-Rules
	 *      in XSLT Specification</a>
	 * @param flushFirst
	 *            Flag indicating the option of overwriting (rather than
	 *            appending to) previous composition.
	 * 
	 * @throws TransformerException
	 */
	public void recomposeTemplates(boolean flushFirst)
		 ...
		 * if (flushFirst) m_templateList = new TemplateList(this);
		 * 
		 * recomposeTemplates();
		 **********************************************************************/
	}
