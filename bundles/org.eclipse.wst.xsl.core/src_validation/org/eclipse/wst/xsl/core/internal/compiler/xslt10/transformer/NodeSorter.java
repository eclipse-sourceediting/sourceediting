 ...
 */
/*
 * $Id: NodeSorter.java,v 1.2 2008/03/28 02:38:16 dacarver Exp $
 */
...
public class NodeSorter {
	...
	protected Vector m_keys; // vector of NodeSortKeys
	 ...
	 */
	public int compare(NodeCompareElem n1, NodeCompareElem n2, int kIndex,
			XPathContext support) throws TransformerException {
		...
		if (k.m_treatAsNumbers) {
			 ...
			 * maxkey) { double n1Num = (double)n1.m_keyValue[kIndex]; double
			 * n2Num = (double)n2.m_keyValue[kIndex]; }
			 */
			else {
			...
			if ((n1Num == n2Num) && ((kIndex + 1) < m_keys.size())) {
				...
				result = ((diff < 0.0) ? (k.m_descending ? 1 : -1)
			...
			/*
			 * Leave this in case we decide to use an array later if (kIndex <
			 * maxkey) { String n1String = (String)n1.m_keyValue[kIndex]; String
			 * n2String = (String)n2.m_keyValue[kIndex]; }
			 */
			else {
	 ...
	 * because it maintains the original document order of the input if the
	 * order isn't changed by the sort.
	 * 
	 * @param a
	 ...
	 * @throws TransformerException
	 */
	public void mergesort(Vector a, Vector b, int l, int r, XPathContext support)
			throws TransformerException {
	 ...
	 * element starting from the left Index. while( (lo < hi0) &&
	 ...
	 *  // if the indexes have not crossed, swap if( lo <= hi ) { swap(v, lo,
	 * hi); ++lo; --hi; } }
	 ...
	 * sort the right partition. if( lo < hi0 ) { QuickSort2( v, lo, hi0,
	 * support ); } } } // end QuickSort2
	 */
	...
	public class NodeCompareElem {
		 ...
		 * maxkey + 1 keys will only hit fairly rarely and therefore, we // will
		 * get the node values for those keys dynamically.
		 */
		protected int maxkey = 2;
		...
		protected Object m_key1Value;

		/** Value from second sort key */
		protected Object m_key2Value;
		 ...
		 * @throws javax.xml.transform.TransformerException
		 */
		public NodeCompareElem(int node)
				throws javax.xml.transform.TransformerException {
			m_node = node;
			...
			if (!m_keys.isEmpty()) {
				...
				/*
				 * Leave this in case we decide to use an array later while
				 * (kIndex <= m_keys.size() && kIndex < maxkey) { NodeSortKey k =
				 * (NodeSortKey)m_keys.elementAt(kIndex); XObject r =
				 * k.m_selectPat.execute(m_execContext, node,
				 * k.m_namespaceContext); if(k.m_treatAsNumbers)
				 * m_KeyValue[kIndex] = r.num(); else m_KeyValue[kIndex] =
				 * r.str(); }
				 */
			} // end if not empty
		}
