 ...
 */
/*
 * $Id: KeyTable.java,v 1.2 2008/03/28 02:38:16 dacarver Exp $
 */
...
import org.eclipse.wst.xsl.core.internal.compiler.xslt10.templates.KeyDeclaration;
...
public class KeyTable {
	 ...
	 *            xsl:key.
	 * @param ref
	 *            The value that must match the value found by the 'match'
	 *            attribute on xsl:key.
	 * @return a set of nodes referenced by the key named <CODE>name</CODE>
	 *         and the reference <CODE>ref</CODE>. If no node is referenced
	 *         by this key, an empty node set is returned.
	 */
	 ...
	 *         function with a XNodeSet
	 */
	private Hashtable getRefsTable() {
		if (m_refsTable == null) {
			// initial capacity set to a prime number to improve hash
			// performance
			...
			while (DTM.NULL != (currentNode = m_keyNodes.nextNode())) {
				try {
					for (int keyDeclIdx = 0; keyDeclIdx < nKeyDecls; keyDeclIdx++) {
						KeyDeclaration keyDeclaration = (KeyDeclaration) keyDecls
								.elementAt(keyDeclIdx);
						XObject xuse = keyDeclaration.getUse().execute(xctxt,
								currentNode, ki.getPrefixResolver());
						...
						if (xuse.getType() != XObject.CLASS_NODESET) {
							...
							while (DTM.NULL != (currentNodeInUseClause = i
								...
								XMLString exprResult = dtm
										.getStringValue(currentNodeInUseClause);
								addValueInRefsTable(xctxt, exprResult,
										currentNode);
							}
						}
					}
	 ...
	 *            XPath context
	 * @param ref
	 *            the value of the use clause of the current key for the given
	 *            node
	 * @param node
	...
	private void addValueInRefsTable(XPathContext xctxt, XMLString ref, int node) {
		...
		if (nodes == null) {
			...
			// Nodes are passed to this method in document order. Since we need
			// to
