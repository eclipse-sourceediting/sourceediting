 ...
 */
/*
 * $Id: ElemValueOf.java,v 1.4 2008/03/28 02:38:15 dacarver Exp $
 */
...
public class ElemValueOf extends ElemTemplateElement {
	...
	/**
	 * Set the "select" attribute. The required select attribute is an
	 * expression; this expression is evaluated and the resulting object is
	...
	/**
	 * Get the "select" attribute. The required select attribute is an
	 * expression; this expression is evaluated and the resulting object is
	 * converted to a string as if by a call to the string function.
	 * 
	 ...
	 * method escapes & and < (and possibly other characters) when outputting
	 * text nodes. This ensures that the output is well-formed XML. However, it
	 * is sometimes convenient to be able to produce output that is almost, but
	 * not quite well-formed XML; for example, the output may include ill-formed
	 ...
	 * string function. The string specifies the string-value of the created
	 * text node. If the string is empty, no text node will be created. The
				...
				try {
					...
					if (transformer.getDebug()) {
						...
						transformer.getTraceManager().fireSelectedEvent(
								current, this, "select", m_selectExpression,
								obj);
