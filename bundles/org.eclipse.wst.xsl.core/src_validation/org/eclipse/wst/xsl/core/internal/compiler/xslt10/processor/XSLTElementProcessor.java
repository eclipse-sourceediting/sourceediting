 ...
 */
/*
 * $Id: XSLTElementProcessor.java,v 1.2 2008/03/28 02:38:16 dacarver Exp $
 */
...
public class XSLTElementProcessor extends ElemTemplateElement {
	 ...
	 * 
	 * @param def
	 *            The element definition object that produced and constrains
	 *            this element.
	 */
	...
	public InputSource resolveEntity(StylesheetHandler handler,
			String publicId, String systemId) throws org.xml.sax.SAXException {
		return null;
	}
	...
	public void endElement(StylesheetHandler handler, String uri,
		...
		if (!getElemDef().getRequiredFound())
			handler.error(XSLTErrorResources.ER_REQUIRED_ELEM_NOT_FOUND,
					new Object[] { getElemDef().getRequiredElem() }, null);
	}
	...
	public void characters(StylesheetHandler handler, char ch[], int start,
		...
		handler.error(XSLTErrorResources.ER_CHARS_NOT_ALLOWED, null, null);// "Characters
																			// are
																			// not
																			// allowed
																			// at
	 ...
	 *            The number of characters to use from the character array.
	 */
	public void ignorableWhitespace(StylesheetHandler handler, char ch[],
	 ...
	 */
	public void processingInstruction(StylesheetHandler handler, String target,
			String data) throws org.xml.sax.SAXException {
	 ...
	 *            The target element where the properties will be set.
	 * @param throwError
	 *            True if it should throw an error if an attribute is not
	 *            defined.
	 * @return the attributes not allowed on this element.
	 * 
	 * @throws TransformerException
	 */
	public Attributes setPropertiesFromAttributes(StylesheetHandler handler,
			String rawName, Attributes attributes, ElemTemplateElement target,
			boolean throwError) throws org.xml.sax.SAXException {

		XSLTElementDef def = getElemDef();
		AttributesImpl undefines = null;
		boolean isCompatibleMode = ((null != handler.getStylesheet() && handler
				.getStylesheet().getCompatibleMode()) || !throwError);
		if (isCompatibleMode)
		...
		for (int i = 0; i < nAttrs; i++) {
			...
			if ((null != attrUri)
					&& (attrUri.length() == 0)
					&& (attributes.getQName(i).startsWith("xmlns:") || attributes
							.getQName(i).equals("xmlns"))) {
			...
			if (null == attrDef) {
				if (!isCompatibleMode) {
					...
					// attribute.
					handler.error(XSLTErrorResources.ER_ATTR_NOT_ALLOWED,
							new Object[] { attributes.getQName(i), rawName },
							null);// "\""+attributes.getQName(i)+"\""
					// + " attribute is not allowed on the " + rawName
					// + " element!", null);
				} else {
					undefines.addAttribute(attrUri, attrLocalName, attributes
							.getQName(i), attributes.getType(i), attributes
							.getValue(i));
				}
			} else {
				...
				boolean success = attrDef.setAttrValue(handler, attrUri,
						attrLocalName, attributes.getQName(i), attributes
								.getValue(i), target);
