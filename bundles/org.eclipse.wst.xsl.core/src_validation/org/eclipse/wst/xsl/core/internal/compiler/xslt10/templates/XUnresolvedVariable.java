 ...
 */
/*
 * $Id: XUnresolvedVariable.java,v 1.3 2008/03/28 02:38:15 dacarver Exp $
 */
...
public class XUnresolvedVariable extends XObject {
	 ...
	 * variable searches for evaluation of expressions. This is -1 if
	 * m_isTopLevel is false.
	 */
	 ...
	 * is primarily used so that forward referencing works with global
	 * variables. An XUnresolvedVariable is initially pushed into the global
	 * variable stack, and then replaced with the real thing when it is
	 * accessed.
	 * 
	 * @param obj
	...
	public XUnresolvedVariable(ElemVariable obj, int sourceNode,
			TransformerImpl transformer, int varStackPos, int varStackContext,
			boolean isGlobal) {
		super(obj);
	...
	public XObject execute(XPathContext xctxt)
		...
		if (!m_doneEval) {
			this.m_transformer.getMsgMgr().error(
					...
					new Object[] { ((ElemVariable) this.object()).getName()
							.getLocalName() });
		}
	...
	/**
	 * Set an index into the variable stack where the variable context ends,
	 * i.e. at the point we should terminate the search.
	 * 
	 * @param bottom
	 *            The point at which the search should terminate, normally zero
