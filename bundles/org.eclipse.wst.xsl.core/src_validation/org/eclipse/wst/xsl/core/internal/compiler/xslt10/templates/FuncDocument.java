 ...
 */
/*
 * $Id: FuncDocument.java,v 1.4 2008/03/28 02:38:15 dacarver Exp $
 */
...
public class FuncDocument extends Function2Args {
	...
	public XObject execute(XPathContext xctxt)
		...
		if (null != arg1Expr) {
			...
			if (XObject.CLASS_NODESET == arg2.getType()) {
				...
				if (baseNode == DTM.NULL) {
					...
					// If the second argument is an empty nodeset, this is an
					// error.
				...
				// %REVIEW% This doesn't seem to be a problem with the
				// conformance
			...
			assertion(null != xctxt.getNamespaceContext(),
		...
		DTMIterator iterator = (XObject.CLASS_NODESET == arg.getType()) ? arg
				.iter() : null;
		int pos = DTM.NULL;

		while ((null == iterator) || (DTM.NULL != (pos = iterator.nextNode()))) {
			XMLString ref = (null != iterator) ? xctxt.getDTM(pos)
					.getStringValue(pos) : arg.xstr();
			...
			// the argument is a node-set, then the result is the union, for
			// each
			// node in the argument node-set, of the result of calling the
			// document
			// function with the first argument being the string-value of the
			// node,
			// and the second argument being a node-set with the node as its
			// only
			// member.
			...
			if (DTM.NULL == docContext) {
				error(xctxt, XSLTErrorResources.ER_NO_CONTEXT_OWNERDOC, null); // "context
																				// does
																				// not
																				// have
																				// an
	 ...
	 * 
	 * @return The document Node pointing to the document at the given URI or
	 *         null
	 * 
	 * @throws javax.xml.transform.TransformerException
	 */
	int getDoc(XPathContext xctxt, int context, String uri, String base)
		...
		try {
			...
			throw new TransformerException(ioe.getMessage(), xctxt
					.getSAXLocator(), ioe);
		...
		try {
			if ((null != uri) && (uri.toString().length() > 0)) {
				newDoc = treeMgr.getSourceTree(source, xctxt.getSAXLocator(),
						xctxt);
			...
			// new Object[]{((base == null) ? "" : base )+uri}); //"Can not load
			// requested doc: "+((base == null) ? "" : base )+uri);
		...
		if (DTM.NULL == newDoc) {
			...
			if (null != diagnosticsString) {
				warn(xctxt, XSLTErrorResources.WG_CANNOT_LOAD_REQUESTED_DOC,
						new Object[] { diagnosticsString }); // "Can not load
																// requested
																// doc: "+((base
																// == null) ? ""
																// : base
				...
				warn(xctxt, XSLTErrorResources.WG_CANNOT_LOAD_REQUESTED_DOC,
						new Object[] { uri == null ? ((base == null) ? ""
								: base)
								+ uri : uri.toString() }); // "Can not load
															// requested doc:
															// "+((base == null)
															// ? "" : base
															// )+uri);
	...
	public void error(XPathContext xctxt, String msg, Object args[])
		...
		TransformerException spe = new TransformerException(formattedMsg, xctxt
				.getSAXLocator());
	...
	protected void reportWrongNumberArgs() throws WrongNumberArgsException {
		throw new WrongNumberArgsException(Messages.createMessage(
				XSLTErrorResources.ER_ONE_OR_TWO, null)); // "1 or 2");
	}
