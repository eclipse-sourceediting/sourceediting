 ...
 */
/*
 * $Id: ElemNumber.java,v 1.4 2008/03/28 02:38:15 dacarver Exp $
 */
...
public class ElemNumber extends ElemTemplateElement {
	...
	private class MyPrefixResolver implements PrefixResolver {
		...
		/**
		 * @see PrefixResolver#getNamespaceForPrefix(String, Node) this
	...
	/**
	 * Set the "count" attribute. The count attribute is a pattern that
	 * specifies what nodes should be counted at those levels. If count
	 * attribute is not specified, then it defaults to the pattern that matches
	 * any node with the same node type as the current node and, if the current
	 * node has an expanded-name, with the same expanded-name as the current
	 * node.
	 * 
	 * @param v
	...
	/**
	 * Get the "count" attribute. The count attribute is a pattern that
	 * specifies what nodes should be counted at those levels. If count
	 * attribute is not specified, then it defaults to the pattern that matches
	 * any node with the same node type as the current node and, if the current
	 * node has an expanded-name, with the same expanded-name as the current
	 ...
	 * Only ancestors that are searched are those that are descendants of the
	 * nearest ancestor that matches the from pattern. For level="any: Only
	...
	/**
	 * Set the "from" attribute. Specifies where to count from. For
	 * level="single" or level="multiple": Only ancestors that are searched are
	 * those that are descendants of the nearest ancestor that matches the from
	 * pattern. For level="any: Only nodes after the first node before the
	 ...
	 * that match the count pattern. If there is no such ancestor, it constructs
	 * an empty list. If the from attribute is specified, then the only
	 * ancestors that are searched are those that are descendants of the nearest
	 ...
	 * current node in document order followed by the element itself; it then
	 * selects from the list those nodes that match the count pattern; it then
	 * maps each node in the list to one plus the number of preceding siblings
	 * of that node that match the count pattern. If the from attribute is
	 ...
	 * containing the current node and all nodes at any level of the document
	 * that are before the current node in document order, excluding any
	 * namespace and attribute nodes (in other words the union of the members of
	 * the preceding and ancestor-or-self axes). If the from attribute is
	 * specified, then only nodes after the first node before the current node
	 * that match the from pattern are considered.
	 * 
	 * @serial
	 */
	...
	/**
	 * Set the "level" attribute. The level attribute specifies what levels of
	 * the source tree should be considered; it has the values single, multiple
	 * or any. The default is single.
	 * 
	 * @param v
	...
	/**
	 * Get the "level" attribute. The level attribute specifies what levels of
	 * the source tree should be considered; it has the values single, multiple
	 * or any. The default is single.
	 * 
	...
	/**
	 * Set the "value" attribute. The value attribute contains an expression.
	 * The expression is evaluated and the resulting object is converted to a
	 * number as if by a call to the number function.
	 * 
	 * @param v
	...
	/**
	 * Get the "value" attribute. The value attribute contains an expression.
	 * The expression is evaluated and the resulting object is converted to a
	 * number as if by a call to the number function.
	 * 
	 ...
	 * the same range of values as xml:lang [XML]; if no lang value is
	...
	/**
	 * Set the "letter-value" attribute. The letter-value attribute
	 * disambiguates between numbering sequences that use letters.
	 * 
	 * @see <a href="http://www.w3.org/TR/xslt#convert">convert in XSLT
	 *      Specification</a>
	 * 
	...
	/**
	 * Get the "letter-value" attribute. The letter-value attribute
	 * disambiguates between numbering sequences that use letters.
	 * 
	 * @see <a href="http://www.w3.org/TR/xslt#convert">convert in XSLT
	 *      Specification</a>
	...
	/**
	 * Set the "grouping-size" attribute. The optional grouping-size specifies
	 * the size (normally 3) of the grouping.
	 * 
	 * @see <a href="http://www.w3.org/TR/xslt#convert">convert in XSLT
	 *      Specification</a>
	 * 
	...
	/**
	 * Get the "grouping-size" attribute. The optional grouping-size specifies
	 * the size (normally 3) of the grouping.
	 * 
	 * @see <a href="http://www.w3.org/TR/xslt#convert">convert in XSLT
	 *      Specification</a>
	...
	public void compose(StylesheetRoot sroot) throws TransformerException {
		...
		if (null != m_groupingSeparator_avt)
			m_groupingSeparator_avt.fixupVariables(vnames, cstate
					.getGlobalsSize());
	...
	/**
	 * Execute an xsl:number instruction. The xsl:number element is used to
	...
	public void execute(TransformerImpl transformer)
		...
		try {
			transformer.getResultTreeHandler().characters(
					countString.toCharArray(), 0, countString.length());
	 ...
	 *            The context in which namespaces in the queries are supposed to
	 *            be expanded.
	 * 
	 ...
	 */
	int findAncestor(XPathContext xctxt, XPath fromMatchPattern,
			XPath countMatchPattern, int context, ElemNumber namespaceContext)
			throws javax.xml.transform.TransformerException {
	...
	private int findPrecedingOrAncestorOrSelf(XPathContext xctxt,
			XPath fromMatchPattern, XPath countMatchPattern, int context,
	...
	XPath getCountMatchPattern(XPathContext support, int contextNode)
		...
		if (null == countMatchPattern) {
			...
			case DTM.ELEMENT_NODE:
				...
				countMatchPattern = new XPath(dtm.getNodeName(contextNode),
						this, resolver, XPath.MATCH, support.getErrorListener());
				break;
			...
			case DTM.ATTRIBUTE_NODE:
				...
				// this);
				countMatchPattern = new XPath("@"
						+ dtm.getNodeName(contextNode), this, this,
			...
			case DTM.TEXT_NODE:
				...
				// this);
				countMatchPattern = new XPath("text()", this, this,
						XPath.MATCH, support.getErrorListener());
				break;
			case DTM.COMMENT_NODE:

				// countMatchPattern =
				// m_stylesheet.createMatchPattern("comment()", this);
				countMatchPattern = new XPath("comment()", this, this,
						XPath.MATCH, support.getErrorListener());
				break;
			case DTM.DOCUMENT_NODE:

				// countMatchPattern = m_stylesheet.createMatchPattern("/",
				// this);
			...
			case DTM.PROCESSING_INSTRUCTION_NODE:
				...
				// this);
				countMatchPattern = new XPath("pi("
						+ dtm.getNodeName(contextNode) + ")", this, this,
						XPath.MATCH, support.getErrorListener());
				break;
			default:
	...
	/**
	 * Given an XML source node, get the count according to the parameters set
	 * up by the xsl:number attributes.
	 * 
	 * @param transformer
	 *            non-null reference to the the current transform-time state.
	 * @param sourceNode
			...
			if (Constants.NUMBERLEVEL_ANY == m_level) {
				...
				if (lastIndex >= 0) {
					...
					for (int i = lastIndex; i >= 0; i--) {
						...
						list[lastIndex - i] = ctable.countNode(xctxt, this,
								target);
					}
				}
			}
	...
	public int getPreviousNode(XPathContext xctxt, int pos)
		...
		if (Constants.NUMBERLEVEL_ANY == m_level) {
			...
			while (DTM.NULL != pos) {
				...
				// dive down to the lowest right-hand (last) child of that
				// sibling.
				...
				if (DTM.NULL == next) {
					...
					if ((DTM.NULL != next)
							&& ((((null != fromMatchPattern) && (fromMatchPattern
									.getMatchScore(xctxt, next) != XPath.MATCH_SCORE_NONE))) || (dtm
				...
				if ((DTM.NULL != pos)
						&& ((null == countMatchPattern) || (countMatchPattern
								.getMatchScore(xctxt, pos) != XPath.MATCH_SCORE_NONE))) {
					break;
				}
			}
	 ...
	 *            Flag indicating to stop after the first node is found
	 *            (difference between level = single or multiple)
	 * @return The number of ancestors that match the pattern.
	 * 
	 * @throws javax.xml.transform.TransformerException
	 */
	NodeVector getMatchingAncestors(XPathContext xctxt, int node,
		...
		while (DTM.NULL != node) {
			if ((null != m_fromMatchPattern)
					&& (m_fromMatchPattern.getMatchScore(xctxt, node) != XPath.MATCH_SCORE_NONE)) {

				// The following if statement gives level="single" different
				// behavior from level="multiple", which seems incorrect
				// according
			...
			if (null == countMatchPattern)
				System.out
						.println("Programmers error! countMatchPattern should never be null!");
	...
	Locale getLocale(TransformerImpl transformer, int contextNode)
		...
		if (null != m_lang_avt) {
			...
			if (null != langValue) {
				...
				if (null == locale) {
					transformer.getMsgMgr().warn(this, null,
							xctxt.getDTM(contextNode).getNode(contextNode),
							XSLTErrorResources.WG_LOCALE_NOT_FOUND,
							new Object[] { langValue }); // "Warning: Could
															// not find locale
															// for
															// xml:lang="+langValue);
	 ...
	 * 
	 * ($objectName$)
	 * @return The number formatter to be used
	 * 
	 * @throws TransformerException
	 */
	private DecimalFormat getNumberFormatter(TransformerImpl transformer,
		...
		String digitGroupSepValue = (null != m_groupingSeparator_avt) ? m_groupingSeparator_avt
				.evaluate(transformer.getXPathContext(), contextNode, this)
				: null;
		...
		String nDigitsPerGroupValue = (null != m_groupingSize_avt) ? m_groupingSize_avt
				.evaluate(transformer.getXPathContext(), contextNode, this)
				: null;
	 ...
	 * @throws TransformerException
	 */
	String formatNumberList(TransformerImpl transformer, long[] list,
			int contextNode) throws TransformerException {
		...
		try {
			...
			String formatValue = (null != m_format_avt) ? m_format_avt
					.evaluate(transformer.getXPathContext(), contextNode, this)
					: null;
			...
			NumberFormatStringTokenizer formatTokenizer = new NumberFormatStringTokenizer(
					formatValue);
			...
			for (int i = 0; i < nNumbers; i++) {
				...
				if (formatTokenizer.hasMoreTokens()) {
					...
					// If the first character of this token is a character or
					// digit, then
					// it is a number format directive.
					if (Character.isLetterOrDigit(formatToken
							.charAt(formatToken.length() - 1))) {
					...
					// then append the formatToken.
						...
						// Since we know the next is a number or digit, we get
						// it now.
						formatToken = formatTokenizer.nextToken();
						numberWidth = formatToken.length();
						numberType = formatToken.charAt(numberWidth - 1);
					} else // only separators left
					{
						...
						// And append any remaining characters to the
						// lastSepString.
				...
				isFirstToken = false; // After the first pass, this should be
										// false
			} // end for loop
	...
	private void getFormattedNumber(TransformerImpl transformer,
			int contextNode, char numberType, int numberWidth,
			long listElement, FastStringBuffer formattedNumber)
			throws javax.xml.transform.TransformerException {

		String letterVal = (m_lettervalue_avt != null) ? m_lettervalue_avt
				.evaluate(transformer.getXPathContext(), contextNode, this)
				: null;
		...
		case 'A':
			if (null == m_alphaCountTable) {
				...
				m_alphaCountTable = (CharArrayWrapper) thisBundle
						.getObject(org.apache.xml.utils.res.XResourceBundle.LANG_ALPHABET);
			}
		...
		case 'a':
			...
			try {
				...
				formattedNumber.append(stringBuf.toString().toLowerCase(
						getLocale(transformer, contextNode)));
			} finally {
		...
		case 0x3042: {
			...
			thisBundle = XResourceBundle.loadResourceBundle(
					org.apache.xml.utils.res.XResourceBundle.LANG_BUNDLE_NAME,
					new Locale("ja", "JP", "HA"));
			...
			if (letterVal != null
				...
				// if (m_lettervalue_avt != null &&
				// m_lettervalue_avt.equals(Constants.ATTRVAL_ALPHABETIC))
				formattedNumber
						.append(int2singlealphaCount(
								listElement,
								(CharArrayWrapper) thisBundle
										.getObject(org.apache.xml.utils.res.XResourceBundle.LANG_ALPHABET)));
		...
		default: // "1"
			DecimalFormat formatter = getNumberFormatter(transformer,
					contextNode);
	 ...
	 * words count using the traditional numbering.
	 * 
	 * @param val
	 *            Value to convert -- must be greater than zero.
	 * @param thisBundle
	...
	protected String tradAlphaCount(long val, XResourceBundle thisBundle) {
		...
		IntArrayWrapper groups = (IntArrayWrapper) thisBundle
				.getObject(org.apache.xml.utils.res.XResourceBundle.LANG_NUMBERGROUPS);

		// array of tables of hundreds, tens, digits...
		StringArrayWrapper tables = (StringArrayWrapper) (thisBundle
				.getObject(org.apache.xml.utils.res.XResourceBundle.LANG_NUM_TABLES));

		// some languages have additive alphabetical notation,
		// some multiplicative-additive, etc... Handle them differently.
		String numbering = thisBundle
				.getString(org.apache.xml.utils.res.XResourceBundle.LANG_NUMBERING);

		// do multiplicative part first
		if (numbering
				.equals(org.apache.xml.utils.res.XResourceBundle.LANG_MULT_ADD)) {
			String mult_order = thisBundle
					.getString(org.apache.xml.utils.res.XResourceBundle.MULT_ORDER);
			LongArrayWrapper multiplier = (LongArrayWrapper) (thisBundle
					.getObject(org.apache.xml.utils.res.XResourceBundle.LANG_MULTIPLIER));
			CharArrayWrapper zeroChar = (CharArrayWrapper) thisBundle
					.getObject("zero");
			int i = 0;
			...
			do {
				...
				// some languages (ie chinese) put a zero character (and only
				// one) when
				// the multiplier is multiplied by zero. (ie, 1001 is 1X1000 +
				// 0X100 + 0X10 + 1)
				// 0X100 is replaced by the zero character, we don't need one
				// for 0X10
				if (val < multiplier.getLong(i)) {
					...
					while (k < groups.getLength()) {
						...
						if (mult / groups.getInt(k) <= 0) // look for right
															// table
							k++;
						else {

							// get the table
							CharArrayWrapper THEletters = (CharArrayWrapper) thisBundle
									.getObject(tables.getString(k));
							...
							table[0] = THEletters.getChar(j - 1); // don't
																	// need this
							...
							char multiplierChar = ((CharArrayWrapper) (thisBundle
									.getObject(org.apache.xml.utils.res.XResourceBundle.LANG_MULTIPLIER_CHAR)))
									.getChar(i);

							// put out the next character of output
							if (lookupIndex < table.length) {
								if (mult_order
										.equals(org.apache.xml.utils.res.XResourceBundle.MULT_PRECEDES)) {
		...
		while (count < groups.getLength()) {
			if (val / groups.getInt(count) <= 0) // look for correct table
				...
				CharArrayWrapper theletters = (CharArrayWrapper) thisBundle
						.getObject(tables.getString(count));
				...
				if (lookupIndex < table.length) {
					...
					// put out the next character of output
					buf[charPos++] = table[lookupIndex]; // left to right or
															// top to bottom
				} else
					return XSLTErrorResources.ERROR_STRING;
	...
	public void callChildVisitors(XSLTVisitor visitor, boolean callAttrs) {
		if (callAttrs) {
			if (null != m_countMatchPattern)
				m_countMatchPattern.getExpression().callVisitors(
						m_countMatchPattern, visitor);
			if (null != m_fromMatchPattern)
				m_fromMatchPattern.getExpression().callVisitors(
						m_fromMatchPattern, visitor);
			if (null != m_valueExpr)
