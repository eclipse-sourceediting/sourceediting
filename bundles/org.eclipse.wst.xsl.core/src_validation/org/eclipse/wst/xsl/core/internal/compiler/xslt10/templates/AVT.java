 ...
 */
/*
 * $Id: AVT.java,v 1.4 2008/03/28 02:38:15 dacarver Exp $
 */
...
public class AVT implements java.io.Serializable, XSLTVisitable {
	 ...
	 * INIT_BUFFER_CHUNK_BITS, INIT_BUFFER_CHUNK_BITS = 7 corresponds size =
	 * 256.
	 */
	...
	/**
	 * Construct an AVT by parsing the string, and either constructing a vector
	 * of AVTParts, or simply hold on to the string if the AVT is simple.
	 * 
	 * @param handler
	...
	public AVT(StylesheetHandler handler, String uri, String name,
			String rawName, String stringedValue, ElemTemplateElement owner)
			throws javax.xml.transform.TransformerException {
				...
				while (tokenizer.hasMoreTokens()) {
					...
					if (t.length() == 1) {
						...
						case ('\''): {
							...
							// just keep on going, since we're not in an
							// attribute template
							buffer.append(t);
						...
						case ('{'): {
							...
							try {
								 ...
								 * lookahead.equals("\'")) { // Error.
								 * Expressions can not begin with quotes. error =
								 * "Expressions can not begin with quotes.";
								 * break; // from switch }
								 */
								else {
									if (buffer.length() > 0) {
										m_parts.addElement(new AVTPartSimple(
												buffer.toString()));
										buffer.setLength(0);
									}
									...
									while (null != lookahead) {
										if (lookahead.length() == 1) {
											...
											case '}': {
												...
												// Proper close of attribute
												// template.
												// Evaluate the expression.
												buffer.setLength(0);

												XPath xpath = handler
														.createXPath(exprBuffer
																.toString(),
																owner);

												m_parts
														.addElement(new AVTPartXPath(
																xpath));
											...
											// part of the template stuff, just
											// add it.
											exprBuffer.append(lookahead);
						...
						case ('}'): {
							...
							if (lookahead.equals("}")) {
								...
								// leave the lookahead to be processed by the
								// next round.
							}
	 ...
	 * @throws javax.xml.transform.TransformerException
	 */
	public String evaluate(XPathContext xctxt, int context,
