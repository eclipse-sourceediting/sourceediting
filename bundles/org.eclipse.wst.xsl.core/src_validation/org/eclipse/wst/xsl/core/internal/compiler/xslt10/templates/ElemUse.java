 ...
 */
/*
 * $Id: ElemUse.java,v 1.3 2008/03/28 02:38:15 dacarver Exp $
 */
...
public class ElemUse extends ElemTemplateElement {
	 ...
	 * both contain the same attribute unless there is a definition of the
	 * attribute set with higher import precedence that also contains the
	 * attribute."
	 * 
	 * @param transformer
	...
	public void applyAttrSets(TransformerImpl transformer,
			StylesheetRoot stylesheet) throws TransformerException {
	...
	private void applyAttrSets(TransformerImpl transformer,
			StylesheetRoot stylesheet, QName attributeSetsNames[])
			throws TransformerException {
			...
			for (int i = 0; i < nNames; i++) {
				...
				if (null != attrSets) {
					...
					for (int k = nSets - 1; k >= 0; k--) {
						ElemAttributeSet attrSet = (ElemAttributeSet) attrSets
								.elementAt(k);
	 ...
	 * attribute sets are specified in the use-attribute-sets attribute. It is
	 * an error if use of use-attribute-sets attributes on xsl:attribute-set
