 ...
 */
/*
 * $Id: ElemSort.java,v 1.3 2008/03/28 02:38:15 dacarver Exp $
 */
...
public class ElemSort extends ElemTemplateElement {
	...
	/**
	 * Set the "select" attribute. xsl:sort has a select attribute whose value
	 * is an expression. For each node to be processed, the expression is
	 * evaluated with that node as the current node and with the complete list
	 * of nodes being processed in unsorted order as the current node list. The
	 ...
	 * value of the select attribute is ., which will cause the string-value of
	 * the current node to be used as the sort key.
	 * 
	 * @param v
	...
	/**
	 * Get the "select" attribute. xsl:sort has a select attribute whose value
	 * is an expression. For each node to be processed, the expression is
	 * evaluated with that node as the current node and with the complete list
	 * of nodes being processed in unsorted order as the current node list. The
	 * resulting object is converted to a string as if by a call to the string
	...
	/**
	 * Set the "lang" attribute. lang specifies the language of the sort keys;
	 * it has the same range of values as xml:lang [XML]; if no lang value is
	...
	/**
	 * Get the "lang" attribute. lang specifies the language of the sort keys;
	 * it has the same range of values as xml:lang [XML]; if no lang value is
	 * specified, the language should be determined from the system environment.
	 * 
	...
	/**
	 * Set the "data-type" attribute. <code>data-type</code> specifies the
	 * data type of the strings; the following values are allowed:
	 * <ul>
	 * <li> <code>text</code> specifies that the sort keys should be sorted
	 * lexicographically in the culturally correct manner for the language
	 * specified by <code>lang</code>. </li>
	 * <li> <code>number</code> specifies that the sort keys should be
	 * converted to numbers and then sorted according to the numeric value; the
	 * sort key is converted to a number as if by a call to the <b><a
	 * href="http://www.w3.org/TR/xpath#function-number">number</a></b>
	 * function; the <code>lang</code> attribute is ignored. </li>
	 * <li> A <a href="http://www.w3.org/TR/REC-xml-names#NT-QName">QName</a>
	 * with a prefix is expanded into an <a
	 ...
	 * </p>
	 * <blockquote> <b>NOTE: </b>The XSL Working Group plans that future
	 * versions of XSLT will leverage XML Schemas to define further values for
	 * this attribute.</blockquote>
	 * 
	 * @param v
	...
	/**
	 * Get the "data-type" attribute. <code>data-type</code> specifies the
	 * data type of the strings; the following values are allowed:
	 * <ul>
	 * <li> <code>text</code> specifies that the sort keys should be sorted
	 * lexicographically in the culturally correct manner for the language
