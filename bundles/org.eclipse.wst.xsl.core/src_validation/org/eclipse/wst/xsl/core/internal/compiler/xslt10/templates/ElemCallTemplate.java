 ...
 */
/*
 * $Id: ElemCallTemplate.java,v 1.3 2008/03/28 02:38:15 dacarver Exp $
 */
...
public class ElemCallTemplate extends ElemForEach {
	...
	/**
	 * An xsl:call-template element invokes a template by name; it has a
	 * required name attribute that identifies the template to be invoked.
	 * 
	 * @serial
	 */
	...
	public void compose(StylesheetRoot sroot) throws TransformerException {
		...
		if ((null != m_templateName) && (null == m_template)) {
			m_template = this.getStylesheetRoot().getTemplateComposed(
					m_templateName);

			if (null == m_template) {
				String themsg = Messages.createMessage(
			...
			for (int i = 0; i < length; i++) {
				...
				int etePos = 0;
				for (ElemTemplateElement ete = m_template.getFirstChildElem(); null != ete; ete = ete
						.getNextSiblingElem()) {
	...
	public void execute(TransformerImpl transformer)
		...
		if (null != m_template) {
						...
						if (ewp.m_index >= 0) {
							if (transformer.getDebug())
								transformer.getTraceManager().fireTraceEvent(
										ewp);
							XObject obj = ewp
									.getValue(transformer, currentNode);
							if (transformer.getDebug())
								transformer.getTraceManager()
										.fireTraceEndEvent(ewp);

							// Note here that the index for ElemWithParam must
							// have been
							// statically made relative to the xsl:template
							// being called,
			...
			transformer.getMsgMgr().error(this,
					...
					new Object[] { m_templateName }); // "Could not find
														// template named:
														// '"+templateName+"'");
		}
