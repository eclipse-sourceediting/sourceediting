 ...
 */
/*
 * $Id: ElemExtensionCall.java,v 1.3 2008/03/28 02:38:15 dacarver Exp $
 */
...
public class ElemExtensionCall extends ElemLiteralResult {
	...
	private ElemExtensionDecl getElemExtensionDecl(StylesheetRoot stylesheet,
		...
		for (int i = 0; i < n; i++) {
			...
			for (ElemTemplateElement child = imported.getFirstChildElem(); child != null; child = child
					.getNextSiblingElem()) {
		...
		try {
			...
			if (null == nsh) {
				if (hasFallbackChildren()) {
					...
					TransformerException te = new TransformerException(Messages
							.createMessage(
									XSLTErrorResources.ER_CALL_TO_EXT_FAILED,
									new Object[] { getNodeName() }));
			...
			try {
				...
				if (hasFallbackChildren())
					...
					if (e instanceof TransformerException) {
						...
						transformer.getErrorListener().warning(
								new TransformerException(e));
					}
				}
			}
	 ...
	 * @throws TransformerException
	 */
	public String getAttribute(String rawName, org.w3c.dom.Node sourceNode,
		...
		if ((null != avt) && avt.getRawName().equals(rawName)) {
			...
			return avt.evaluate(xctxt, xctxt.getDTMHandleFromNode(sourceNode),
					this);
		}
