 ...
 */
/*
 * $Id: ElemTemplate.java,v 1.3 2008/03/28 02:38:15 dacarver Exp $
 */
...
public class ElemTemplate extends ElemTemplateElement {
	...
	/**
	 * The owning stylesheet. (Should this only be put on the template element,
	 * to conserve space?)
	 * 
	 * @serial
	 */
	...
	/**
	 * Get the stylesheet composed (resolves includes and imports and has
	 * methods on it that return "composed" properties.
	 * 
	...
	/**
	 * Set the "match" attribute. The match attribute is a Pattern that
	 * identifies the source node or nodes to which the rule applies. The match
	 * attribute is required unless the xsl:template element has a name
	 * attribute (see [6 Named Templates]). It is an error for the value of the
	...
	/**
	 * Get the "match" attribute. The match attribute is a Pattern that
	 * identifies the source node or nodes to which the rule applies. The match
	 * attribute is required unless the xsl:template element has a name
	 * attribute (see [6 Named Templates]). It is an error for the value of the
	 * match attribute to contain a VariableReference.
	...
	/**
	 * Set the "priority" attribute. The priority of a template rule is
	 * specified by the priority attribute on the template rule. The value of
	 * this must be a real number (positive or negative), matching the
	 * production Number with an optional leading minus sign (-).
	 * 
	...
	/**
	 * Get the "priority" attribute. The priority of a template rule is
	 * specified by the priority attribute on the template rule. The value of
	 * this must be a real number (positive or negative), matching the
	 * production Number with an optional leading minus sign (-).
	 * 
	 ...
	 * number of params and variables that can be declared in the template at
	 * one time.
	 */
	 ...
	 * identifiers for the arguments. The position of a given qname in the list
	 * is the argument ID, and thus the position in the stack frame.
	 */
	...
	public void compose(StylesheetRoot sroot) throws TransformerException {
		...
		if (null != m_matchPattern)
			m_matchPattern.fixupVariables(vnames, sroot.getComposeState()
					.getGlobalsSize());
	 ...
	 * xsl:template element is the template that is instantiated when the
	 * template rule is applied.
	 * 
	 * @param transformer
