...
public class XMLPropertySourceAdapter implements INodeAdapter, IPropertySource, IPropertySourceExtension {
	...
	// derive categories from CMDataTypes; disabled until display strings can be planned
	...
	protected IPropertyDescriptor createDefaultPropertyDescriptor(String attributeName, boolean hideOnFilter) {
		// The descriptor class used here is also used in updatePropertyDescriptors()
	 ...
	 * - if the value does not exist, an editable combo box is returned
	 * - if the value exists but is not one in the enumerated list of value,
	 *     a combo box featuring the current and correct values is returned
	 * - if the value exists and it is a valid value, a combo box featuring
	 *     the correct values with the current one visible is returned
	 */
	...
	/**
	 * Creates a property descriptor for an attribute with a FIXED value
	 * - if the value does not exist, an editable combo box is returned
	 * - if the value exists but is not the fixed/default value,
	 *     a combo box featuring the current and correct value is returned
	 * - if the value exists and it is the fixed/default value, no cell editor
	 *     is provided "locking" the value in
	 */
	...
	protected IPropertyDescriptor[] createPropertyDescriptors() {
		...
		if (attributes != null) {
			for (int i = 0; i < attributes.getLength(); i++) {
				...
				// if metainfo is present for this attribute, use the CMAttributeDeclaration to derive a descriptor
				if (attrMap != null) {
				...
				// be consistent: if there's metainfo, use *that* as the descriptor ID
				if (attrDecl != null) {
	...
	protected void updatePropertyDescriptors() {
		...
		// New descriptor list that will become fDescriptors after all processing is done
			...
			for (int i = 0; i < attrMap.getLength(); i++) {
				...
				for (int j = 0; j < fDescriptors.length; j++) {
					...
					if (sameName) {
						...
						// Update the descriptor for this CMAttributeDeclaration (only enumerated values get updated for now)
		...
		// Remove descriptors for attributes that aren't present AND aren't known through metainfo,
		// do this by only reusing existing descriptors for attributes that are present or declared
