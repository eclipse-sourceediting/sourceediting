...
import org.eclipse.wst.contentmodel.modelquery.CMDocumentManagerListener;
...
public class JFaceNodeAdapter implements IJFaceNodeAdapter {
	...
	public class CMDocumentManagerListenerImpl implements CMDocumentManagerListener {
		...
		public void cacheCleared(org.eclipse.wst.contentmodel.util.CMDocumentCache cache) {
	...
	protected Image createXMLImageDescriptor(String imageResourceName) {
		ImageDescriptor imageDescriptor = XMLEditorPluginImageHelper.getInstance().getImageDescriptor(imageResourceName);
	...
	/**
	 * @deprecated moved to JFaceNodeAdapterForHTML because it seems more fitting
	 */
	protected Image createHTMLImageDescriptor(String imageResourceName) {
		return createXMLImageDescriptor(imageResourceName);
	}
	...
	protected Image createImage(Object object) {
		...
		switch (node.getNodeType()) {
			...
			case Node.TEXT_NODE :
				{ // actually, TEXT should never be seen in the tree
					image = createXMLImageDescriptor(XMLEditorPluginImages.IMG_OBJ_ELEMENT);
					break;
				}
	 ...
	 */
	public org.eclipse.wst.contentmodel.modelquery.CMDocumentManagerListener getCMDocumentManagerListener() {
	...
	public void notifyChanged(INodeNotifier notifier, int eventType, Object changedFeature, Object oldValue, Object newValue, int pos) {
		...
		while (iterator.hasNext()) {
			Object listener = iterator.next();
			if ((listener instanceof StructuredViewer) && (eventType == INodeNotifier.STRUCTURE_CHANGED || (eventType == INodeNotifier.CHANGE && changedFeature == null))) {
				...
				if (structuredViewer.getControl() != null && !structuredViewer.getControl().isDisposed()) {
					RefreshOutlineJob newJob = new RefreshOutlineJob(getDisplay(), ResourceHandler.getString("JFaceNodeAdapter.0"), structuredViewer, notifier); //$NON-NLS-1$
					newJob.schedule();
				}
			}
