...
public class ReconcileStepForContentModel extends StructuredReconcileStep {
	...
	protected boolean ranInitialValidate = false;
	...
	/**
	 * Mark the INodeNotifier (Node) and all children of the INodeNotifier passed in.
	 * @param notifier
	 */
	...
	/**
	 * Forces the IReconcilerAdapters for XMLNodes overlapping the given region to
	 * "validate" their Nodes.
	 * @param startOffset
	 * @param length
	 */
	protected IReconcileResult[] validate(int startOffset, int length) {
		...
		if (!ranInitialValidate) {
			initialValidate();
			ranInitialValidate = true;
		}
		else {
			...
			// sometimes for single key type length can be 0 (startOffset == endOffset)
