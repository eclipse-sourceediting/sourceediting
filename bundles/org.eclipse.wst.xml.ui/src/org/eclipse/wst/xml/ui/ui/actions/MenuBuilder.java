...
public class MenuBuilder {
	...
	/*
	 protected void createPropertyGrouping(IMenuManager menu, List actionList)
	 {                                                                       
	 ...
	 {        
	 groupName = ((NodeAction)action).getGroupName();
	 }         
	 if (groupName == null)
	 {
	 groupName = "";
	 }
	 MenuData menuData = menuDataTable.lookupOrCreate(groupName, "");
	 menuData.childList.add(action);          
	 }                        
	 ...
	 {                                               
	 menuManager.add((Action)o);
	 }
	 else if (o instanceof MenuData)
	 {                                               
	 MenuData childMenuData = (MenuData)o;
	 MenuManager childMenuManager = new MenuManager(childMenuData.name);
	 ...
	 public MenuDataTable()
	 {
	 root = lookupOrCreateMenuData(null, null);
	 }

	 protected MenuData lookupMenuData(String name)
	 {          
	 String key = name != null ? name : "";           
	 return (MenuData)menuDataTable.get(key);        
	 }

	 protected MenuData lookupOrCreateMenuData(String name, String parentName)
	 {                                
	 String key = name != null ? name : "";
	 MenuData menuData = (MenuData)menuDataTable.get(key);
	 if (menuData == null)
	 {
	 menuData = new MenuData(name, parentName);
	 menuDataTable.put(key, menuData);
	 }                                   
	 return menuData;
	 }  

	 public MenuData getRoot()
	 {
	 return root;
	 }           
	 }
	 ...
	 public List childList = new Vector();

	 MenuData(String name, String parentName)
	 {
	 this.name = name;
	 ...
	 protected void sort()
	 {           
	 Object[] array = childList.toArray();
	 if (array.length > 0 )
	 {
	 Arrays.sort(array, comparator);
	 }
	 childList = Arrays.asList(array);   
