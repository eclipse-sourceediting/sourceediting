...
public class CommonNamespaceInfoTable extends TableViewer {
	...
	protected int getColumnWidth(int column) {
		...
		switch (getColumnCode(column)) {
			...
			case COLUMN_NAMESPACE_URI :
				{
					// Size columns differently when location hint text label is displayed
					if (showLocationText) {
						result = 10;
					}
					else {
	...
	// {
	//  return (column == COLUMN_CHECKBOX) ? checkBoxCellEditor : textCellEditor;
	// }
	...
	public CommonNamespaceInfoTable(Composite parent, int style, int visibleRows, boolean showLocationText) {
		...
		for (int i = 0; i < columnPropertiesArray.length; i++) {
			...
			if ((columnPropertiesArray[i]).equals(LABEL_LOCATION_HINT)) {
				if (showLocationText) {
					...
					// Comment this out until we solve the alignment/(space for image being allocated
					// to prefix column) ......
		...
		/*CellEditor[] cellEditors = new CellEditor[columnPropertiesArray.length];
		 for (int i = 0; i < columnPropertiesArray.length; i++)
		 {
		 cellEditors[i] = getCellEditor(i);
		 }
		...
		public Image getColumnImage(Object object, int columnIndex) {
			...
			if (columnCode == COLUMN_LOCATION_HINT) {
				...
				if (info.locationHint == null || info.locationHint.trim().equals("")) { //$NON-NLS-1$
					// Comment this out until we solve the alignment/(space for image being allocated
					// to prefix column) ......
					//          result = XMLEditorPluginImageHelper.getInstance().getImage(XMLEditorPluginImages.IMG_OBJ_WARNING_OBJ);
				}
			}
	...
	/*
	 ...
	 {
	 if (property.equals(LABEL_CHECKBOX))
	 {
	 return true;
	 }
	 else if (property.equals(LABEL_PREFIX))
	 {
	 return true;
	 }
	 return false;
	 ...
	 else if (property.equals(LABEL_NAMESPACE_URI))
	 {
	 column = 2;
	 }
