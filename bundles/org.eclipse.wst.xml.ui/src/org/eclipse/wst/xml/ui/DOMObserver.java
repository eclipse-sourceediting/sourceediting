...
import org.eclipse.wst.contentmodel.modelqueryimpl.CMDocumentLoader;
import org.eclipse.wst.contentmodel.modelqueryimpl.InferredGrammarBuildingCMDocumentLoader;
...
/**
 * This class is used to observe changes in the DOM and perform occasional'scans' to deduce information.
 * We use a delay timer mechanism to ensure scans are made every couple of seconds to avoid performance problems.
 * Currently this class is used to keep track of referenced grammar uri's within the document ensure that they
 * are loaded by the CMDocumentManager.  We might want to generalize this class to perform other suplimental 
 * information gathering that is suitable for 'time delayed' computation (error hints etc.).     
 */
...
public class DOMObserver {
	...
	public void init() {
		// CS: we seem to expose an XSD initialization problem when we do this immediately
				...
				case INodeNotifier.ADD :
					{
						if (newValue instanceof Element) {
							//System.out.println("ADD (to " + ((Node)notifier).getNodeName() + ") " + ((Element)newValue).getNodeName() + " old " + oldValue);
