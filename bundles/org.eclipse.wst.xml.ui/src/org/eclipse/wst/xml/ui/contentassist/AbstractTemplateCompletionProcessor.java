...
/**
 * A completion processor that computes template proposals and works with AbstractContentAssistProcessor
 */
abstract public class AbstractTemplateCompletionProcessor extends TemplateCompletionProcessor {
	...
	protected TemplateContextType getContextType(ITextViewer viewer, IRegion region) {
		...
		// context type id, so just use that value instead of trying to determine 
	...
	protected ICompletionProposal createProposal(Template template, TemplateContext context, Region region, int relevance) {
		// CustomTemplateProposal turns the additional information to content fit for HTML
		return new CustomTemplateProposal(template, context, region, getImage(template), relevance);
	}

	/**
	 * Return the template store to use with this template completion processor
	 * @return TemplateStore
	 */
	abstract protected TemplateStore getTemplateStore();

	/**
	 * Return the template context registry to use with this completion processor
