...
import org.eclipse.wst.contentmodel.util.CMDocumentCacheListener;
...
public class ReconcileStepAdapterForXML extends AbstractReconcileStepAdapter implements CMDocumentCacheListener, IReleasable {
	...
	// to yeild the thread control to the next thread
	...
	protected ModelQuery getModelQuery(Node node) {
		...
		return (node.getNodeType() == Node.DOCUMENT_NODE) ? ModelQueryUtil.getModelQuery((Document) node) 
															: ModelQueryUtil.getModelQuery(node.getOwnerDocument());
	}
	...
	public void processNotification(INodeNotifier notifier, int eventType, Object changedFeature, Object oldValue, Object newValue, int pos, IProgressMonitor monitor) {
		...
		// directives,
		//       and schema and namespace related attributes (DOCTYPE, taglib, xmlns,
		// xsi...)
		...
		if (eventType == INodeNotifier.CHANGE && changedFeature instanceof Element) {
			...
			if (newNode.getNodeType() == Node.DOCUMENT_TYPE_NODE || newNode.getNodeName().equals("jsp:directive.taglib")) { // $NON-NLS-1$ //$NON-NLS-1$
				fNeedsRefreshAll = true;
			}
			else {
	...
	public void notifyChanged(INodeNotifier notifier, int eventType, Object changedFeature, Object oldValue, Object newValue, int pos) {
		...
		synchronized (notifications) {
			...
			if (!notifications.contains(newEvent))
				notifications.add(newEvent);
		}
	}
	...
	protected IReconcileResult[] reconcile(Object o, IProgressMonitor monitor) {
		...
		if (o instanceof XMLNode) {
			...
			if (mq != null) {
				caseSensitive = isCaseSensitive(xmlNode);
	...
	protected void processNotifications(IProgressMonitor monitor) {
		...
		synchronized (notifications) {
			...
			events = (NotificationEvent[]) notifications.toArray(new NotificationEvent[0]);
			notifications.clear();
		}
		...
		if (elementDecl != null) {
			...
			for (int i = 0; i < attrs.getLength(); i++) {
				...
				if (!shouldIgnore(attr)) {
					...
					// iFix V511i
					// CMVC 272647, attributes with namespace prefix get marked
			...
			// if there are missing required attributes, create annotations for
			// them
			...
			Object[] additionalFixInfo = {
				new Integer(startTagOffset),
				new Integer(startTagLength),
				new Integer(endTagOffset),
				new Integer(endTagLength)
			};
