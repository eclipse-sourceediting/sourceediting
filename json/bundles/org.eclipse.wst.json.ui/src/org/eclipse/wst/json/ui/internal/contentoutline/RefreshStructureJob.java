 ...
 *     Angelo Zerr <angelo.zerr@gmail.com> - copied from org.eclipse.wst.xml.ui.internal.contentoutline.RefreshStructureJob
...
class RefreshStructureJob extends Job {
	...
	private synchronized Object[] getRefreshRequests() {
		IJSONNode[] toRefresh = (IJSONNode[]) fRefreshes.toArray(new IJSONNode[fRefreshes.size()]);
		fRefreshes.clear();
	...
	private synchronized Object[] getUpdateRequests() {
		IJSONNode[] toUpdate = (IJSONNode[]) fUpdates.toArray(new IJSONNode[fUpdates.size()]);
		fUpdates.clear();
