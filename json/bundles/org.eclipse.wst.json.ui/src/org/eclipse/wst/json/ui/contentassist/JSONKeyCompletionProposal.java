...
public class JSONKeyCompletionProposal extends CustomCompletionProposal {
	...
	/**
	 * Adds a {@link LinkedPosition} to its own position group. This group is
	 * then added to the model
	 * 
	 * @param model
	 *            the linked model for this proposal
	 * @param document
	 *            the document the content assist is operating upon
	 * @param offset
	 *            the offset to establish the {@link LinkedPosition}
	 * @param length
	 *            the length of the {@link LinkedPosition}
	 * @throws BadLocationException
	 */
	private void addPosition(LinkedModeModel model, IDocument document,
			int offset, int length) throws BadLocationException {
		final LinkedPositionGroup group = new LinkedPositionGroup();
		group.addPosition(new LinkedPosition(document, offset, length,
				LinkedPositionGroup.NO_STOP));
		model.addGroup(group);
	}

	public Point getSelection(IDocument document) {
		// Attempt to return the selection based on the selected region from the
		// LinkedModeUI
